...
# Filter to reasonable prices (drops ~15% of listings at the extremes)
n_before = len(df)
...
The classical bias-variance tradeoff says: more complexity eventually hurts. But in [Chapter 6](lec06-trees.qmd), we saw that random forests — averages of many overfit trees — improve with more trees, not fewer. That pattern departs from the U-shaped curve above.

The resolution: averaging independent overfit models (bagging) reduces variance without increasing bias. Each individual tree is high-variance, but their average is stable. The ensemble principle — that averaging independent overfit models can escape the classical tradeoff — motivates much of modern machine learning. (We demonstrated the effect in [Chapter 6](lec06-trees.qmd): adding more trees to a random forest reduced test error monotonically, with no U-shape.)
...
- **Forward references:** [Chapter 8](lec08-sampling.qmd) (bootstrap), [Chapter 12](lec12-regression-inference.qmd) (regression inference), [Chapter 16](lec16-backtesting.qmd) (temporal validation), [Chapter 17](lec17-automl-llms.qmd) (ensembles deep dive).
