...
The classical bias-variance tradeoff says: more complexity eventually hurts. But there's an important exception. If you train many overfit models on different random subsets of the data and *average* their predictions, the average can be more accurate than any individual model — even though each model is overfit. Averaging reduces variance without increasing bias, escaping the U-shaped curve.

In [Chapter 7](lec07-trees.qmd), we'll see this principle in action with **random forests**: ensembles of hundreds of overfit decision trees whose average prediction is remarkably stable. Adding more trees to the ensemble never hurts — a fundamental departure from the polynomial story above.
...
- **Averaging overfit models can escape the classical tradeoff.** If independent models are each overfit but their errors point in different directions, averaging cancels noise without losing signal. [Chapter 7](lec07-trees.qmd) demonstrates this with random forests.
...
You are responsible for: train/test split, cross-validation (how it works and why it's better than a single split), overfitting concept, bias-variance tradeoff (the classical U-shape), Lasso concept (what it does, not the optimization details), distribution shift, and the principle that averaging overfit models reduces variance (demonstrated with random forests in Chapter 7).
