---
title: "When Validation Isn't Enough"
execute:
  enabled: true
jupyter: python3
---
...
These are three different deployment failures with one common root: *the world you tested on isn't the world you deployed into, and sometimes the model itself reshapes that world.* Models are predictions about the world, and predictions change the world. This chapter is about three reasons a validated model fails after deployment: **temporal leakage** (you saw the future in your test set), **feedback loops** (your prediction changes the outcome), and **Goodhart's law** (the metric you optimized stops measuring the thing you cared about). Each has its own diagnostics, its own fixes, and its own warning signs — and recognizing them is what separates a model that looks good on paper from a model that survives contact with the world.
...
Cathy O'Neil calls the worst cases **Weapons of Math Destruction** (WMDs) — predictive models with three properties:^[O'Neil, *Weapons of Math Destruction*, Crown, 2016.]
...
"When a measure becomes a target, it ceases to be a good measure" (attributed to Charles Goodhart, 1975; popularized by Marilyn Strathern). Whenever people or systems are rewarded for optimizing a metric, they find ways to hit the target without achieving the goal the metric was supposed to measure. Goodhart's law is the mechanism behind many of the feedback loops you just saw: the measure gets gamed — by people, by institutions, or by the optimization itself.

The pattern appears everywhere once you see it.

- **Hospital readmission penalties.** The US Centers for Medicare and Medicaid Services penalize hospitals with high 30-day readmission rates. The metric was designed to measure care quality. Instead, some hospitals responded by declining to admit the sickest patients in the first place — keeping readmission rates low by selecting healthier patients, not by providing better care.
- **School test scores.** When teacher evaluations depend on standardized test scores, the incentive shifts from teaching understanding to teaching the test. Scores rise; learning does not.
- **Emissions testing.** Volkswagen installed software that detected when a vehicle was being emissions-tested and switched to a low-emission mode. The metric — emissions during the official test — was hit cleanly. The goal — low emissions during actual driving — was not.
- **Citations and h-index.** Academic careers increasingly depend on citation counts. Researchers respond by salami-slicing papers, publishing incremental work, and forming citation cartels. Citation counts climb; the goal they were meant to proxy (scientific quality) does not.
- **p-hacking.** We saw in [Chapter 10](lec10-hypothesis-testing.qmd) that the $p < 0.05$ threshold was designed to measure evidence strength. When careers and publications depend on clearing that bar, researchers (often unconsciously) run analyses until they find significance. p-hacking is Goodhart's law applied to scientific inference.

## The machine-learning reading

Goodhart's law has a specifically mathematical version that sits at the heart of overfitting. Go back to [Chapter 6](lec06-validation.qmd). When you train a model, you optimize a **training loss** — say, mean squared error on the training set. But the training loss is a *proxy* for what you actually care about: **test loss**, the model's error on new data from the same distribution. For a model with modest capacity, the proxy and the goal move together, and you can minimize training loss safely. For a model with enough capacity to memorize the training set — a high-degree polynomial, a deep decision tree, a neural net with millions of parameters — the proxy and the goal diverge. You can drive training loss to zero while test loss gets worse. The training metric stopped being a good measure of the thing you cared about precisely *because* you optimized it so hard.

This is why cross-validation exists. Holding out a validation set and tracking validation loss is a way of resisting Goodhart: you optimize on the training loss, but you decide *when to stop* based on a metric the optimizer never saw. It's the same idea as "the independent auditor doesn't report to the CEO."

But even cross-validation is only a partial defense. If you repeatedly tune hyperparameters against the validation set — if you let the validation set *become* the target — the validation loss itself becomes a gamed metric. This is why serious ML practice distinguishes **training / validation / test** sets: the test set is used *once*, after all decisions are locked, precisely because any metric you optimize against long enough stops measuring what you wanted.

## Asking the Goodhart question

The pattern is the same whether the optimizer is a gradient-descent run or a hospital administrator: a metric designed to *measure* performance becomes a target to *optimize*, and optimization pressure corrupts the metric. Whenever you use a number to make decisions, ask: **could the people or systems being measured change their behavior to hit the target without achieving the underlying goal?** If yes, you have a Goodhart problem, and you need a second measure — one the optimizer can't see, or one that captures a different aspect of the goal — to tell you when the first one has been gamed.
...
- **Temporal leakage.** Random splits leak future information into training. Use temporal splits, walk-forward validation, and lag features engineered from past-only data.
- **Distribution shift.** Models trained on normal conditions fail during unprecedented events. No cross-validation fully fixes this; widen prediction intervals during high-variance regimes and treat extreme-event predictions as extrapolation.
- **Feedback loops.** When a model's predictions change the world, the test set no longer represents the deployment environment. Ask: *how do the predictions change the outcome distribution?* If they do, you have a feedback loop.
...
- **Goodhart's law.** When a measure becomes a target, it ceases to be a good measure. This is the mechanism behind overfitting (training loss as proxy for test loss), p-hacking, hospital readmission gaming, emissions cheating, and citation inflation.
...
- **Feedback loop (performative prediction):** a causal arrow from the model's prediction back to the outcome — prediction → action → new data → new prediction. The test set no longer represents the deployment environment.
