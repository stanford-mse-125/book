...
:::{.callout-tip}
## Think About It
Why does averaging many overfitting trees produce a good model? Hint: each tree overfits to *different* noise because it sees different bootstrap samples and different feature subsets.
:::
...
- Decision trees partition the feature space into rectangular regions by recursively splitting on single features.
- Deep trees overfit; random forests fix this by averaging many trees trained on different bootstrap samples.
- Gradient boosting trains trees sequentially on residuals, connecting to the gradient descent idea from Lecture 13.
- AutoML automates model selection but cannot frame the question, check data quality, or interpret results.
- LLMs produce working code quickly but systematically fail at checking assumptions, recognizing confounding, and calibrating uncertainty.
- Decomposing an analysis into small, verifiable prompts lets you catch AI errors before they propagate.
