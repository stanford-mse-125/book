---
title: "Lecture 11: Multiple Testing and Correlation"
execute:
  enabled: true
jupyter: python3
---
...
They find 30 "significant" results and write a report:
*"These 30 players benefit from load management."*
...
:::{.callout-note}
## The Idol of the Qu
Salsburg warns about exactly this in *The Lady Tasting Tea*. By the mid-20th century, hypothesis testing had become a ritual: collect data, compute a p-value, publish if $p < 0.05$, move on. Researchers stopped asking whether the question was worth testing and started treating the 0.05 threshold as a stamp of scientific truth. Salsburg calls this the "idol of the Qu" — the worship of quantitative methods for their own sake, divorced from scientific thinking. The intern's analysis is a textbook case: 431 p-values, no scientific question, no correction for the fact that most "discoveries" are noise. (See Salsburg, *The Lady Tasting Tea*, Ch. 20.)
:::
...
Recall from Lecture 10 that we test at significance level $\alpha = 0.05$, accepting a 5% false positive rate for a *single* test. But what happens when we run hundreds of tests?
...
This is the **multiple testing** problem: running many hypothesis tests on the same dataset. Let's do exactly what the intern did.
...
where $m$ is the number of tests. (This follows from linearity of expectation and holds regardless of dependence between tests.)
...
print(f"Std dev under null (Binomial): {sd_under_null:.1f}")
...
Bonferroni killed every result. But is that too aggressive? Maybe some players really do benefit from rest — we just can't tell because Bonferroni demands an extremely low p-value for each test. Can we find a middle ground that allows a controlled fraction of false discoveries?
...
This guarantees that among your "discoveries," at most a fraction $\alpha$ are false.
:::
...
Use Bonferroni when *any* false positive is costly (e.g., drug safety). Use BH when you can tolerate some false discoveries (e.g., screening genes for further study). This connects to the dollar-sign decisions from Lecture 10: the stakes determine the correction.
...
- **Incentive misalignment.** Careers are built on novel, statistically significant findings. Null results don't get published, don't earn grants, and don't lead to promotions. This creates strong pressure (often unconscious) to find significance.

:::{.callout-important}
## Definition: Pre-registration
**Pre-registration** means publicly committing to your research question, data collection plan, and analysis plan *before* looking at the data. This eliminates researcher degrees of freedom by fixing the garden of forking paths in advance. A pre-registered analysis is a single, pre-specified test — not the survivor of many. Pre-registration doesn't prevent exploratory analysis; it simply requires labeling it honestly as exploratory (hypothesis-generating) rather than confirmatory (hypothesis-testing).
:::
...
The **Pearson correlation** $r$ measures the *linear* association between two variables (you may have seen this in MS&E 120):
...
The **reason** for rest is **confounded** with the **outcome** of rest. We'll formalize this idea in Lecture 18 using causal diagrams (DAGs), which make it precise to say exactly which variables are confounders.
...
This is **Simpson's paradox** (named after Edward Simpson, 1951): the aggregate comparison shows rest *hurts* by several points, but within individual players, the effect is essentially zero. The aggregate pattern was entirely driven by *who* gets rest, not by rest itself.
...
The fix is to compare each player to *themselves*. For each player-season, we subtract their average points. Now a value of +2 means "this player scored 2 points above their own average." This removes player quality from the comparison.
...
Both halves of today's lecture share a theme: statistical results can be deeply misleading when you don't account for the structure of your analysis.
...
4. **Correlation is not causation.** The negative correlation between rest and points exists because
   of *who* gets rest, not because rest hurts performance. Confounding variables drive the pattern.
...
In Lecture 12, we'll use regression to estimate the effect of rest while controlling for player quality, schedule difficulty, and fatigue — and we'll test whether that coefficient is statistically significant.
