...
**Think about it:** If rest has zero effect on *every* player, how many of our tests would you expect to come back "significant" at $\alpha = 0.05$?
...
**Think about it:** If the null is true for every player, what should the histogram of p-values look like?
...
This is the **Bonferroni correction**. It controls the **family-wise error rate (FWER)** — the probability of *any* false positive across all tests. It's conservative — meaning it may miss real effects — but it guarantees the probability of *any* false positive stays below $\alpha$, no matter how the tests are correlated.
...
Now let's quantify this with the **Pearson correlation** $r$, which measures the *linear* association between two variables (you may have seen this in MS&E 120):

$$r = \frac{\sum (x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\sum (x_i - \bar{x})^2 \sum (y_i - \bar{y})^2}}$$

$r$ ranges from $-1$ to $+1$. Important: $r = 0$ means no *linear* relationship, not no relationship at all. Two variables can be strongly related (e.g., quadratic) and still have $r$ near 0.
...
**Think about it:** Before reading on — why might more rest be associated with fewer points? What's different about players who get extended rest? Discuss with your neighbor.
...
## The surprise: Simpson's paradox in rest data
