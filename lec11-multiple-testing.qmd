...
[Lecture 10](lec10-hypothesis-testing.qmd) ran a two-sample t-test on continuous outcomes — two arms of the ACTG trial, $\sigma$ estimated from the data. Here we shift to one sample (one player) and a binary outcome (made or missed). Two changes at once, both riding on a single property of the Bernoulli distribution: **the mean determines the variance**. A shot taken with true probability $p$ has variance $p(1-p)$ — a function of $p$ alone. So the moment $H_0$ pins down the mean as $p_0$, the standard error is locked in at $\sqrt{p_0(1-p_0)/n}$. No control group is needed to estimate variability, and nothing in the player's own data is needed either. That free standard error — Bernoulli's gift — is what makes the one-sample z-test for a proportion work.
...
::: {.callout-note}
## How is this different from the t-test?
The t-test from [Chapter 10](lec10-hypothesis-testing.qmd) and the z-test for a proportion are the **same recipe**: take the standardized distance from the null,
...
## Going deeper: when your budget $K$ is fixed
