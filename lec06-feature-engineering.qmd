...
*Think about it:* why do polynomial features on bedrooms alone give such small improvements? What other sources of variation in price are we ignoring?

## Section 3: Interaction terms
...
## Section 4: A different approach — decision trees
...
*Think about it:* the tree picks its own split points. Look at the first split — does it match your intuition about what matters most for price?

## Section 5: Random forests as benchmark
...
## Section 6: Classification with trees
...
## Section 7: LLM-based featurization
...
LLM featurization is a powerful modern technique: instead of building a natural language processing pipeline from scratch, you write a prompt and let the model extract structured information. The resulting features are imperfect — LLMs can be wrong — but cross-validation tells you whether they help.
...
- **One-hot encoding** — converting a categorical variable with $k$ categories into $k-1$ binary (0/1) columns, one per category (drop one to avoid collinearity)
- **Polynomial features** — powers of a variable ($x^2, x^3, \ldots$) that let a linear model capture curved relationships
- **Interaction term** — a product of two features (e.g., bedrooms $\times$ $\mathbb{1}_{\text{Manhattan}}$) that lets one feature's effect depend on another
- **Decision tree** — a model that recursively splits the feature space into regions, predicting the mean (regression) or majority class (classification) in each region
- **Random forest** — an ensemble of many decision trees trained on random subsets of data and features; predictions are averaged (regression) or majority-voted (classification)
- **Weak supervision** — using noisy, programmatic labels (e.g., from an LLM) instead of expensive hand labels

### Key ideas (one sentence each)

- One-hot encoding lets categorical variables enter a regression as shifts in the intercept for each group.
- Polynomial and interaction features make a "linear" model capture nonlinear patterns — it's linear in $\beta$, not in $x$.
- Decision trees learn if/then splits automatically, handling nonlinearities and interactions without manual engineering.
- Random forests average many trees to reduce overfitting — more trees never hurts.
- LLM featurization converts unstructured text into structured features using a single prompt, enabling weak supervision at scale.
