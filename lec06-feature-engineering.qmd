...
In [Chapter 5](lec05-regression.qmd), we fit a regression predicting Airbnb prices from bedrooms and bathrooms. That model ignored most of the information in the dataset: borough, room type, listing descriptions. **Feature engineering** transforms raw data into inputs a model can use. We start with classical techniques (dummies, polynomials, interactions), meet decision trees that engineer features automatically, and end with a modern approach: using an LLM to extract structured features from unstructured text.
...
The problem: encoding stages as 1--4 assumes the outcome changes by the same amount per stage *and* that the spacing between stages is uniform. Neither is true. One-hot encoding (a separate dummy for each stage) lets each stage have its own effect, free from these assumptions. Alternatively, **boolean threshold encoding** creates features like "Stage II or higher" and "Stage III or higher," which respect the ordering without forcing a linear relationship.
...
Imputation replaces missing values with plausible substitutes (median, mean, or a model-based prediction). Without a missingness indicator, the model can't distinguish real values from imputed ones. Always pair imputation with an indicator column when missingness might be informative (MNAR or MAR — see Chapter 2).
:::
...
A model that memorizes training data looks perfect on those same observations but fails on new ones. To get an honest measure of prediction quality, we hold out 30% of listings as a **test set** that the model never sees during fitting. [Chapter 7](lec07-trees-validation.qmd) develops this idea fully.
...
The tree's test performance slightly outperforms the linear model with hand-engineered interactions. Let's visualize what the tree learned using `plot_tree`.
...
- Next: [Chapter 7](lec07-trees-validation.qmd) asks "how do we know these features aren't just overfitting?" — introducing train/test splits, cross-validation, and the bias-variance tradeoff.
...
- **Missing values as features** — missing values can be turned into features. For categoricals, treat "missing" as its own category. For numerics, pair an imputed value with a binary missingness indicator so the model can learn whether missingness itself predicts the outcome.
