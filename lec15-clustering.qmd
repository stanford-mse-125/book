...
K-means uses **Euclidean distance** (L2) by default, but the choice of distance metric shapes what "close" means — and therefore what clusters you find.

- **Manhattan distance** (L1): the sum of absolute differences along each axis. It treats each feature independently rather than blending them, which can be more appropriate when features are on different scales or represent genuinely different quantities.
- **Cosine similarity**: measures the angle between two vectors, ignoring their magnitude. This is widely used for text data and any setting where the *direction* of a profile matters more than its *size*.

**Concrete example.** Consider two Airbnb listings: a luxury studio ($400/night, 1 bedroom, 10 reviews) and a budget 3-bedroom ($120/night, 3 bedrooms, 200 reviews). In Euclidean distance, these might land in the same cluster because the price difference is modest after standardization. But in cosine similarity, they point in very different directions — one is "expensive and small," the other is "cheap, large, and popular." The metric you choose determines which similarities the algorithm sees.

Standard k-means only supports Euclidean distance (because the mean minimizes squared Euclidean error). To use other metrics, you'd need a variant like **k-medoids**, which replaces the mean with the most central observed point and works with any distance function.
:::
