...
The basketball half showed how feature choice shapes clusters when stakes are low. We now look at four published case studies — single-cell biology, asthma, depression, and stellar astronomy — that turn on the same point at higher stakes. Two are successes, one is a cautionary failure, one is mixed. All four are real studies in their fields' top journals.

### PCA, then k-means: a common recipe in the wild

The cases below use a few flavors of clustering, but one recipe dominates the wild: run PCA first, then cluster on the PC scores. Why? K-means uses Euclidean distance, which behaves badly in high dimensions when features are many, correlated, or on mixed scales — distances become noisy and uninformative. PCA projects the data onto a small orthogonal basis that captures most of the variance, so distances in the reduced space track meaningful differences. Then k-means (or a graph-based variant) finds groups in the reduced representation.
...
3. **Cluster** (k-means or a related algorithm) on the $k$-dimensional PC scores.

The marquee published example below — Tabula Muris — uses exactly this pipeline on tens of thousands of genes.

### Tabula Muris: clustering as the scaffolding of a cell atlas

The Tabula Muris Consortium profiled the gene expression of 100,605 individual cells across 20 mouse organs.[^tabula-muris] No cell came pre-labeled with its type. The pipeline: PCA on the most variable genes to reduce each cell's expression vector from tens of thousands of genes to a few dozen components, then graph-based (Seurat / Louvain) clustering on the PC scores. The result was around 50 clusters per organ.

Why this counts as a *good* use of clustering: each cluster was then annotated against known **marker genes** — genes that decades of cell-biology research had already pinned to specific cell types. When a cluster's top-expressed genes lined up with the marker set for, say, hepatocytes, the cluster got that name. Almost every named cell type in mouse anatomy came back out. The clustering itself didn't reveal cell types; it organized the data well enough that biologists could verify each group against an existing standard.
...
Haldar and colleagues clustered three asthma cohorts on 8 clinical and physiologic features: age of asthma onset, sex, atopic status, body-mass index, peak-flow variability, FEV1 bronchodilator response, sputum eosinophil count, and exhaled nitric oxide.[^haldar] The cohorts: a primary-care sample (n = 184), a refractory secondary-care sample (n = 187), and a longitudinal randomized trial (n = 68). The method: Ward's hierarchical clustering to pick $k$, then k-means with that $k$.

Two clusters showed up in both the primary-care and the refractory cohorts: **early-onset atopic** asthma and **obese non-eosinophilic** asthma. Two additional clusters appeared only in the refractory cohort and were *discordant* — symptoms and inflammation moved in opposite directions: a **symptom-predominant** group (lots of symptoms, little eosinophilic inflammation) and an **inflammation-predominant** group (eosinophilic, few symptoms).

The validation was a randomized trial. Patients in the inflammation-predominant cluster were assigned to be managed either by symptoms (the standard of care) or by inflammation markers. The inflammation-managed arm cut exacerbations from roughly 3.5 to 0.4 per patient per year. Symptom-predominant patients managed by symptoms received substantially less inhaled steroid than they would have under inflammation-marker management. Cluster membership predicted *which treatment helped which patient*. That's a harder test than "the clusters look biologically interpretable": different treatments worked in different clusters, so the clusters mapped onto a real joint in the disease.
...
The Tabula Muris and Haldar studies show what success looks like. The next study shows what failure looks like — and it published first.

Drysdale and colleagues claimed to discover **four biotypes of depression** in 2017.[^drysdale] The pipeline: canonical correlation analysis (CCA) linking resting-state fMRI connectivity features to clinical symptom scores in 1,188 patients, then hierarchical clustering on the CCA-projected fMRI scores. The four resulting biotypes were claimed to be diagnosable from fMRI alone with 82–93% sensitivity and specificity, and to predict response to transcranial magnetic stimulation (TMS) therapy.

Two years later, Dinga and colleagues attempted to replicate the pipeline on an independent sample of 187 patients.[^dinga] Permutation tests on the canonical correlations — the same correlations that Drysdale's pipeline pivoted on — returned p-values of 0.64 and 0.99. Cross-validated correlations on held-out data dropped to about zero. The four-cluster solution itself was no better than chance on standard cluster-validity tests (Calinski-Harabasz p = 0.45, silhouette p = 0.19 under the null). Cluster assignments were unstable: leaving one subject out at a time changed which cluster many subjects got assigned to.

The mechanism: CCA can always find linear combinations of two high-dimensional feature sets that look correlated in the data they were fit on, even when no structure exists in the population. With enough features and not enough samples, the optimization step *manufactures* apparent structure. Without a permutation test or out-of-sample replication, you cannot tell that structure from a real signal. Drysdale's original analysis didn't do either step. Dinga's did, and the structure dissolved.
...
Garcia-Dias and colleagues ran k-means on 153,847 stellar spectra from the APOGEE survey (resolution $R \approx 22{,}500$), using the normalized flux at each wavelength as the input features — the raw spectra, not derived astrophysical parameters like temperature, surface gravity, or metallicity.[^garcia-dias]

The result was mixed. Some known stellar populations came back: dwarfs vs. giants, red-clump and red-giant-branch stars, bulge vs. halo. But the recovery was partial, the algorithm was sensitive to initialization, and the authors flagged the core limitation themselves: *"a discrete classification in flux space does not result in a neat organisation in the parameters' space."*

The lesson: the underlying objects — distinct stellar populations differing in mass, age, and chemical composition — are physically real. Yet k-means in raw-flux space only partially recovered them, because Euclidean distance between two spectra is not the same notion of similarity as distance between two stars in the parameter space astronomers actually care about. The clustering algorithm did its job correctly; the *similarity metric* — encoded by the feature choice — was the wrong one for the question.
...
Four cases, four fields, four verdicts.
...
- **Drysdale → Dinga (bad).** Pretty clusters in a high-dimensional, small-sample CCA pipeline failed every check an independent group could think to apply.
- **APOGEE (mixed).** The objects were real; the features encoded the wrong notion of similarity. The clustering algorithm wasn't broken; the choice of distance was.

All four turn on the same axis. The algorithm always returns groups. Whether to trust those groups depends on what you can check *outside* the clustering: a curated reference, a follow-on experiment, a replication on independent data, or a known set of objects whose structure you can compare to. Cluster-validity statistics measured on the same data the clusters were fit on don't qualify.
