...
**In practice: always pass `n_init=10` to k-means, and use the default `k-means++` initializer.** This is what sklearn's docs recommend and what made the rest of this chapter's figures reproducible. The diagnostic above shows what happens when you *don't*.
...
Subsequent work — Chouldechova 2017 ("Fair prediction with disparate impact") and Corbett-Davies et al. 2017 ("Algorithmic decision making and the cost of fairness") — showed that no risk score can simultaneously be calibrated across groups and have equal false-positive rates across groups when base rates differ. The COMPAS controversy turned out to be an instance of a general impossibility result. There is no algorithmic fix; the trade-off between competing fairness criteria is a value judgment, not a math problem.
:::
