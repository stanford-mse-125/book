...
The Indiana Pacers' front office wants to sign a wing who can space the floor *and* attack closeouts. The traditional 5-position scheme (PG, SG, SF, PF, C) is no help: Stephen Curry and Russell Westbrook are both "point guards" but play nothing alike. What is the real taxonomy of NBA players in 2024?
...
Before clustering, we must commit to a notion of *distance* — a number that says how far apart two players are. For shot-mix data, the natural choice is **Euclidean distance** on the standardized features: square the differences zone by zone, add them up, take a square root. Two players are "close" when their five shot-share percentages line up.

The choice of distance is itself a value judgment. Euclidean distance on shot mix says two players are similar when their *recipes* match, regardless of height, defense, rebounding, or even total shot volume. A star and a role player who take the same proportions from each zone look identical to this metric.
...
K-means is an iterative algorithm. We start by picking $k$ **centroids** — points in feature space that will represent the clusters — and then repeat two steps until assignments stop changing:
...
The form mirrors the SSE we minimized in regression (Chapter 4): there, the "prediction" for each point was a single number (the mean) or a fitted line; here, the "prediction" is its cluster's centroid. K-means partitions the data into clusters and replaces each point with its centroid, just as regression replaces each $y_i$ with $\hat y_i$.
:::

The two steps above are the two halves of an **alternating minimization** of SSE: step 2 fixes the centroids and re-assigns each point to its nearest one (the SSE-minimizing assignment); step 3 fixes the assignments and sets each centroid to the mean of its points (the SSE-minimizing centroid, and the source of the "mean" in k-means). This alternating-minimization pattern is the same engine that fits PCA in Chapter 14.
...
The **elbow method** plots SSE against $k$. We look for an "elbow" — a value of $k$ beyond which adding clusters yields little further reduction in SSE.
...
The five archetypes are not the *only* way to cluster these 317 players. Same algorithm, same players, different feature set, different answer. Two alternative feature recipes illustrate the point.
...
K-means starts from random initial centroids. Different starts can yield different final partitions: the algorithm finds a **local optimum**, not the global one.
...
To expose the instability we run k-means 10 times with different seeds, each using just *one* random start per run (`n_init=1`). To compare two clusterings we use the **Adjusted Rand Index (ARI)**. The ARI asks: across all pairs of players, do the two clusterings agree on whether each pair lands in the same cluster? An ARI of 1 means perfect agreement; an ARI of 0 means agreement no better than chance; the ARI can be negative when two clusterings disagree more than chance would predict (rare but possible).
...
raw_h = KMeans(n_clusters=3, random_state=42, n_init=10).fit_predict(X)
compas['cluster_hist'] = relabel_by_key(raw_h, compas, 'priors_count', ascending=True)
```
...
- **K-means** finds local-optimum partitions by minimizing within-cluster squared distance from each point to its centroid. The study guide below carries the formula.
...
Chapter 17 returns to feature-choice stakes from the AI angle — when an LLM picks features for you, who is making the value judgment?

Chapters 18-19 ask a different question: not "what groups exist?" but "did this intervention cause that outcome?" The shift is from prediction to **causal inference**.
...
- **SSE (sum of squared errors):** the k-means objective — total squared distance from each point to its cluster centroid. (Sklearn calls this `inertia_`; the SSE label is consistent with regression in Chapter 4.)
- **Silhouette score:** mean over points of $s(i) = (b(i) - a(i)) / \max(a(i), b(i))$, where $a(i)$ is the mean within-cluster distance and $b(i)$ is the mean distance to the nearest other cluster. Range $[-1, 1]$; higher is better.
- **Elbow method:** plot SSE against $k$ and look for diminishing returns.
...
- **Adjusted Rand Index (ARI):** measures agreement between two clusterings, adjusted for chance. $\mathrm{ARI} = 1$ for identical clusterings; $\mathrm{ARI} = 0$ for chance agreement; $\mathrm{ARI} < 0$ when two clusterings disagree more than chance would predict (rare but possible).
