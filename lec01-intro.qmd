...
**Inference** asks: *what can we conclude about the population?* If a hospital's ERR is 1.05, is that a real signal or just noise from a small sample? Inference quantifies uncertainty — it tells you how confident to be. A single number without an uncertainty estimate is dangerous: Zillow's algorithm predicted home prices without prediction intervals and lost $881 million. In Act 2, we'll build the tools for inference: confidence intervals, hypothesis tests, and p-values.
...
**Act 1: Build Models** (Lectures 1–7) — Explore data, clean it, and build predictive models. We'll use regression, feature engineering, and decision trees on real datasets.
...
The mean is close to 1.0 — most hospitals are near the expected rate. But the spread matters: some hospitals are well above or below.
...
heart_failure = readmissions[readmissions['Measure Name'] == 'HF - Heart Failure']
print(f"Heart failure rows: {heart_failure.shape[0]:,}")
heart_failure[['Facility Name', 'State', 'Excess Readmission Ratio']].head()
```

The expression inside the brackets — `readmissions['Measure Name'] == 'HF - Heart Failure'` — produces a True/False value for each row. Only the True rows are kept. This pattern of filtering by a condition is how you slice data in pandas.
