...
Meanwhile, training $R^2$ keeps inching up — from about 0.21 at degree 1 to 0.31 at degree 6 — while each additional degree buys less and less. Degrees beyond 6 add essentially nothing to training $R^2$ while making the extrapolation wilder. We'll develop formal tools to detect overfitting in [Chapter 6](lec06-validation.qmd).
...
The polynomial parade of the previous section tried to capture curvature from the $X$ side by piling on higher-order terms in the additive structure — and extrapolated into absurdity. The problem was the additivity itself: each new term adds a correction, and nothing in the math keeps predictions positive or prevents them from swinging below zero.
