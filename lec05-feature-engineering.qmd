...
A second reason to ignore the coefficient: the feature isn't earning its place in the model. It doesn't meaningfully improve predictions, *and* its causal interpretation is broken. A feature that carries neither predictive nor causal weight has no business influencing decisions.
...
With the imputed column and indicator in place, we can verify the invariance claim directly: fit with two different imputation constants and check that the predictions agree row by row.
...
Let's fit a log-log model on our data so the bottom rows of the table aren't abstract. Use `accommodates` (the listing's sleeping capacity) — a feature where "10% more" is a meaningful quantity.
...
The distribution is right-skewed: the mean sits noticeably above the median, the tell-tale sign of a long upper tail. The model underpredicts some expensive listings by hundreds of dollars while most residuals cluster near zero. That asymmetry hints at a second problem we'll see explicitly below — the *spread* of residuals is not constant across the range of fitted values.
