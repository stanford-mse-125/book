...
$$X^T X \beta = X^T y \qquad \Longrightarrow \qquad \beta = (X^T X)^{-1} X^T y \quad \text{(when } X \text{ has linearly independent columns)}$$
...
The coefficient on `number_of_reviews` is *negative*. The association is real: listings with more reviews do tend to have lower prices in this data, holding other features constant. But reading it as a recommendation — "get more reviews to lower your price" — is wrong. This is **reverse causation**: price influences how often a listing sells, bookings drive review counts, so reviews sit downstream of price. A \$60/night studio might host 200 guests a year; a \$350/night loft might host 30. More guests means more reviews. The regression faithfully reports the backward flow but has no window into the direction of influence. An analyst who stops at the coefficient — or worse, recommends a host strategy based on it — has confused association with causation.

A second reason to ignore the coefficient: the feature isn't earning its place in the model. It doesn't meaningfully improve predictions, *and* its causal interpretation is confounded. A feature that carries neither predictive nor causal weight has no business influencing decisions.
...
This section works through four moves, from simple to elaborate. We start with missing values — a one-column trick that turns an awkward data-cleaning problem into a feature. Then polynomials let the slope bend. Interactions let the slope depend on another variable. And in the next section, log transforms reshape the response itself.

## Missing values as features

A missing `cleaning_fee` is not just an annoyance — it carries information. A host who leaves the cleaning fee blank might bundle cleaning into the nightly rate, might be a casual operator who skipped optional fields, or might expect the guest to tidy up before checkout. We don't know the reason from the data alone, but whatever the mechanism, listings with no listed fee are systematically cheaper. The missingness itself is a signal about the listing, and we'd like the model to see it.
...
The code cell below verifies the invariance: fit with $c = 0$ and $c = 50$, and confirm the predictions match.
