...
For the two-feature model above, $X$ has three columns: the all-ones column, `bedrooms`, and `bathrooms`. The prediction for listing $i$ is the inner product of row $i$ of $X$ with $\beta$ — the same "sum of feature times coefficient" we wrote out in Chapter 4, now packaged so the whole dataset's predictions are produced at once by the matrix–vector product $X\beta$.
