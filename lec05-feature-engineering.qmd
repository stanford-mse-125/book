...
There is a subtlety. The three dummy columns sum to 1 for every row — exactly the all-ones column $\mathbf{1}$ that the intercept already contributes. Adding the three dummies together reproduces a column that is already in $X$, so the extra column is *linearly dependent* on the existing columns. Recall from Chapter 4 that the span is the set of all reachable predictions; the column space is the span of $X$'s columns. Adding a linearly dependent column doesn't grow the span at all — it just creates a collinearity, and the normal equations become unsolvable ($X^TX$ is singular). The fix: drop one category, called the **reference level**, so the remaining dummy columns are linearly independent of the intercept.
...
A missing `cleaning_fee` is not just an annoyance — it carries information. A host who leaves the cleaning fee blank is often a casual operator, a budget listing, or someone who absorbs cleaning into the nightly rate. The missingness itself is a signal about the listing, and we'd like the model to see it.
...
Listings with a missing cleaning fee are noticeably cheaper on average — the missingness isn't random noise, it's tracking something real about the listing. The approach: impute the missing numeric value, then add a binary `fee_missing` column. The model can then learn whether "missing" predicts a different price from any observed fee.

For least squares, the **mean** is the right value to impute. Replacing a missing cell with the mean minimizes the squared-error contribution of that cell on average, which is exactly the loss the regression is trying to reduce.

```{python}
# Impute the cleaning fee with its mean and record which rows were filled in
mean_fee = df['cleaning_fee'].mean()
df['cleaning_fee_imputed'] = df['cleaning_fee'].fillna(mean_fee)
df['fee_missing'] = df['cleaning_fee'].isna().astype(int)

print(f"Mean cleaning fee (used for imputation): ${mean_fee:.2f}")
print(f"\nSample rows:")
...
print()
print(f"fee_missing coefficient: ${m_fee_ind.coef_[-1]:+.2f}")
```

The imputed fee pushes $R^2$ up sharply — the fee itself is a strong price signal. Adding the `fee_missing` indicator contributes a smaller additional bump, and the indicator's coefficient is negative: even after accounting for the imputed fee, listings that left it blank are associated with lower prices. The model has learned something from the *pattern* of missingness that raw imputation alone can't capture.
