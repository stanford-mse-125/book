...
Percentages tell a different story. Four of the five boroughs land between 51% and 80% per bedroom. Still a real range — but meaningfully closer to "one shared number" than the dollar column is. The Bronx remains an outlier either way: 23% per bedroom against its small base price.

So the question is: can we replace five separate bedroom-slope interaction terms with one proportional coefficient? For four of the five boroughs, approximately yes. For the Bronx, no — Bronx is genuinely a different market. That trade-off is exactly what the log transform offers.
...
A bedroom in Manhattan adds many more dollars than a bedroom in the Bronx, even though both boroughs share the same *proportional* increase in the log model. That single multiplier captures four of the five boroughs approximately — Manhattan, Brooklyn, Queens, and Staten Island all sat in the 51-80% range in the interaction coefficients. The Bronx remains the real outlier at 23%; no single shared percentage will fix that. The log transform has traded five interaction coefficients for one number, at the cost of mis-predicting the one borough that genuinely behaves differently.
...
The top 5% of listings — roughly 1,500 out of 29,000 — account for nearly half of the level model's total squared error. The model is spending most of its coefficient budget fighting to fit the expensive tail.

Does that fight pay off? We can answer by breaking the data into price deciles and computing the mean absolute error within each decile, for both the level model and the log model (with a Duan smearing correction to backtransform log predictions into dollars honestly).

```{python}
# Precompute predictions once.
yhat_level_b = model_level_b.predict(X_level_b)

# Duan smearing correction: mean(exp(log residuals)) fixes the Jensen bias
# introduced by naive exp() of log-model predictions.
log_pred_b = model_loglevel.predict(X_level_b)
log_resid_b = log_prices - log_pred_b
smearing = np.mean(np.exp(log_resid_b))
yhat_log_dollars = np.exp(log_pred_b) * smearing
...
ax.set_xlim(0, decile_df[['level MAE', 'log MAE']].values.max() * 1.18)
plt.tight_layout()
plt.show()
```

Two patterns stand out. First, the top decile ($251-$999) has about $170 MAE under *either* model — the expensive tail is essentially unfittable with these features, regardless of the transformation. The level model's focus on that tail buys it nothing. Second, in deciles 5-7 (roughly $90-$150 per night, where most listings live) the log model's MAE is 75-81% of the level model's. That is a real improvement of about $10 per listing in the middle of the distribution.
...
Ordinary least squares minimizes the mean squared error. So how can the log transform *improve* the mean squared error? Hint: among which functions is the level model's MSE the minimum? Is $\exp(x^T\beta)$ one of them?
:::
...
x0 = rng.uniform(0, 10, n_syn)
y0 = 1 + 2 * x0 + rng.normal(0, 2, n_syn)
m0 = LinearRegression().fit(x0.reshape(-1, 1), y0)
r0 = y0 - m0.predict(x0.reshape(-1, 1))
axes[0, 0].scatter(m0.predict(x0.reshape(-1, 1)), r0, alpha=0.6, s=18, color='#2171b5')
...
x1 = rng.uniform(0, 10, n_syn)
y1 = 1 + 2 * x1 + rng.normal(0, 0.35 * (x1 + 0.5), n_syn)
m1 = LinearRegression().fit(x1.reshape(-1, 1), y1)
r1 = y1 - m1.predict(x1.reshape(-1, 1))
axes[0, 1].scatter(m1.predict(x1.reshape(-1, 1)), r1, alpha=0.6, s=18, color='#d62728')
...
# Bottom-left: CURVE (true quadratic fit with a line)
x2 = rng.uniform(-3, 3, n_syn)
y2 = x2**2 + rng.normal(0, 1.0, n_syn)
m2 = LinearRegression().fit(x2.reshape(-1, 1), y2)
r2 = y2 - m2.predict(x2.reshape(-1, 1))
axes[1, 0].scatter(m2.predict(x2.reshape(-1, 1)), r2, alpha=0.6, s=18, color='#ff7f0e')
...
x3 = rng.uniform(0, 10, n_syn)
group = rng.choice([0, 1], size=n_syn)
y3 = 1 + 2 * x3 + np.where(group == 1, 6, -6) + rng.normal(0, 1.0, n_syn)
m3 = LinearRegression().fit(x3.reshape(-1, 1), y3)
r3 = y3 - m3.predict(x3.reshape(-1, 1))
axes[1, 1].scatter(m3.predict(x3.reshape(-1, 1)), r3, alpha=0.6, s=18, color='#2ca02c')
...
axes[0].set_title('Level model residuals — right-skewed')
...
The level-model residuals are visibly right-skewed. The mean sits well above the median — the sign of a long upper tail of positive residuals on expensive listings the model systematically underpredicted. That skew was a clue: the same right-tail pattern is visible in the raw price distribution, and what we did to the price distribution (log transform) also fixed the residual distribution. The log-model residuals are much more symmetric, with mean and median nearly coinciding near zero. The residuals told us which transformation we needed, if we had known what to look for.
...
1. **Is the coefficient real or noise?** The bathrooms coefficient is about \$43, but with a different sample it might be \$39 or \$47. We'll quantify the uncertainty with confidence intervals in [Chapter 12](lec12-regression-inference.qmd).
