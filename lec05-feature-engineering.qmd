...
Dollar premiums span a 5× range: from $16 in the Bronx to $81 in Manhattan. The interaction model needed a separate slope for every borough to fit that spread.

Percentages tell a different story. Four of the five boroughs land between 51% and 80% per bedroom. Still a real range — but meaningfully closer to "one shared number" than the dollar column is. The Bronx is the one borough that doesn't fit either picture: 23% per bedroom, \$16 per bedroom, against a small base price.

So the question is: can we replace five separate bedroom-slope interaction terms with a single proportional coefficient? For four of the five boroughs, approximately yes — they're close enough to a shared percentage that one number can summarize them. The Bronx will be left under-fit as the price of that simplification. The log transform is the tool that makes this trade available.

## From additive to multiplicative

The percentage clue from the interaction coefficients points the way. A single shared "proportional bedroom premium" would need a model that thinks in multiplications, not additions. The polynomial parade of the previous section tried to capture curvature by piling on higher-order terms in the additive structure — and extrapolated into absurdity. The problem was the additivity itself: each new term adds a correction, and nothing in the math keeps predictions positive or prevents them from swinging below zero.
...
## Why least squares fights the wrong battle

The interaction-coefficient hint is one reason the log transform was the right move. There is a second, independent reason, and it is baked into the least-squares objective itself.

Start with the raw price distribution.
...
The median listing costs $100 per night, but the distribution runs all the way out to $999 — a fat right tail of expensive listings an order of magnitude above the center. When the level model minimizes squared error in dollars, that tail is where most of its error budget ends up.

```{python}
# How much of the level-model's squared error comes from the most expensive listings?
resid_level_b = prices.values - model_level_b.predict(X_level_b)
sse_level_b = resid_level_b ** 2

order = np.argsort(-prices.values)  # sort listings from most to least expensive
sse_sorted = sse_level_b[order]
total_sse = sse_level_b.sum()

for pct in [5, 10, 20]:
    n = int(len(prices) * pct / 100)
    share = sse_sorted[:n].sum() / total_sse * 100
    print(f"Top {pct:2d}% most expensive listings: {share:.1f}% of total squared error")
```

If every listing contributed the same share, the top 5% would account for 5% of the squared error. Instead they account for nearly half — the top 5% of listings absorb almost ten times their "fair share." The model is spending most of its coefficient budget fighting to fit the expensive tail.

Does that fight pay off? We can answer by breaking the data into price deciles and computing the mean absolute error within each decile for both the level model and the log model. One subtlety: the log model predicts $\log(y)$, so we have to exponentiate to get a dollar prediction — and naive `np.exp()` of a log regression's fitted values systematically *underpredicts* the mean on the dollar scale (the arithmetic mean of $\exp(\log y)$ is larger than $\exp$ of the mean of $\log y$). A standard fix is to multiply by the average of $\exp(\text{log residuals})$, which rescales the predictions to match the mean. We apply that correction here so the level/log comparison is honest.
...
```{python}
# Horizontal bar chart, two bars per decile
fig, ax = plt.subplots(figsize=(8.5, 5.5))
y_pos = np.arange(len(decile_df))
bar_height = 0.38
max_mae = decile_df[['level MAE', 'log MAE']].values.max()
label_offset = max_mae * 0.012  # offset as fraction of axis width so labels clear the bar ends
...
ax.set_yticklabels([f"D{r['decile']} ({r['price range']})" for _, r in decile_df.iterrows()],
                   fontsize=9)
ax.invert_yaxis()  # decile 1 (cheapest) at top
...
Two patterns stand out. First, the top decile ($251-$999) has roughly \$170 MAE for the level model and \$180 for the log — expensive listings are hard to fit either way. Our features describe bedroom count and borough, but not the amenities, sub-neighborhood, property type, or seasonality that drive the difference between a \$300 and a \$900 listing. Neither model has what it would take to predict those well, and the level model's concentration of attention on them buys it roughly nothing. Second, in deciles 4-7 (roughly \$76-\$150 per night, where most listings live) the log model's MAE is 74-89% of the level model's. That is a real improvement of \$5-$10 per listing across the middle of the distribution.

The mechanism is straightforward. Squared-dollar error treats a \$50 miss on a \$100 listing (50% off — a disaster) the same as a \$50 miss on a \$500 listing (10% off — mild). The level model cannot distinguish these, so it distributes its attention by *absolute* dollars, which means the expensive tail dominates. The log transform puts errors on a proportional scale — a 50% miss is a 50% miss whether the listing is \$100 or \$500 — which matches how we actually judge prediction quality.

:::{.callout-tip}
## Think About It

The level model minimizes mean squared error in dollars — it is by definition the best fit among linear functions $\beta_0 + \beta_1 \cdot \text{bedrooms} + \cdots$. And yet the log model just beat it on MAE across most of the distribution. How is that possible? Is the log model's backtransformed prediction, $\exp(\beta_0 + \beta_1 \cdot \text{bedrooms} + \cdots)$, a linear function of bedrooms and borough? If not, what does that tell you about what "least squares minimizes MSE" actually promises?
:::
...
Now look at the residuals from our own full model.
...
```{python}
# Fit a log-transformed version of the full model for comparison
model_log_full = LinearRegression().fit(X_full, np.log(prices))
log_residuals_full = np.log(prices.values) - model_log_full.predict(X_full)

fig, axes = plt.subplots(1, 2, figsize=(9.5, 4))

# Level model residuals (dollars)
mean_level = residuals_full.mean()
median_level = np.median(residuals_full)
# Round first so labels don't display "-0" for tiny negatives
mean_level_int = int(round(mean_level))
median_level_int = int(round(median_level))
axes[0].hist(residuals_full, bins=60, edgecolor='white', alpha=0.75, color='#d62728')
axes[0].axvline(median_level, color='#2171b5', lw=2, ls='--',
                label=f'median ≈ {median_level_int}')
axes[0].axvline(mean_level, color='black', lw=2, ls='--',
                label=f'mean ≈ {mean_level_int}')
axes[0].set_yscale('log')  # log-y makes the long positive tail visible
axes[0].set_xlabel('Residual ($)')
axes[0].set_ylabel('Count (log scale)')
axes[0].set_title('Level model residuals — long positive tail')
axes[0].legend(loc='upper right', fontsize=9)

# Log model residuals (log units)
mean_log = log_residuals_full.mean()
median_log = np.median(log_residuals_full)
# Round to 2 decimals first so labels don't display "-0.00"
mean_log_disp = round(mean_log, 2) + 0.0
median_log_disp = round(median_log, 2) + 0.0
axes[1].hist(log_residuals_full, bins=60, edgecolor='white', alpha=0.75, color='#2171b5')
axes[1].axvline(median_log, color='#d62728', lw=2, ls='--',
                label=f'median ≈ {median_log_disp:+.2f}')
axes[1].axvline(mean_log, color='black', lw=2, ls='--',
                label=f'mean ≈ {mean_log_disp:+.2f}')
axes[1].set_xlabel('Residual (log units)')
axes[1].set_ylabel('Count')
axes[1].set_title('Log model residuals — more symmetric')
...
The normal equations pin the level-model residual *mean* to exactly zero — a few pages ago that was a feature, but here it hides a problem. The *median* sits well below zero (around -\$17), which is the sign of a long upper tail of positive residuals on expensive listings the model systematically underpredicted. Plotting the count on a log y-axis makes that tail visible: notice how many listings have residuals above +$100, and how few have residuals that far negative. That asymmetry was a clue — the same right-tail pattern was already visible in the raw price distribution. What we did to the price distribution (the log transform) also fixes the residual distribution: the log-model residuals on the right panel are much more symmetric, with mean and median nearly coinciding near zero. The residuals told us which transformation we needed, if we had known what to look for.
