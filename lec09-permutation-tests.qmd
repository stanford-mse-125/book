...
This is the **permutation test**. The distribution of fake effects we get from shuffling is called the **null distribution** — it shows what the test statistic looks like when the null hypothesis (no effect) is true.
...
*Think about it:* If the drug really works, where will our observed effect of 71 fall relative to the null distribution? Will it be in the middle or out in the tail?
...
We compute a **two-sided** p-value because we would be surprised by a large effect in *either* direction — a drug that dramatically helps or dramatically hurts patients would both be noteworthy.
...
*Think about it:* Why do we add 1 to both the numerator and denominator? Because with a finite number of permutations, we can't know for certain that the true p-value is exactly zero. The conservative estimator honestly reflects our uncertainty.
