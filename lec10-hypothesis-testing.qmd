...
There's a subtler worry behind this: when the $p < 0.05$ threshold becomes a *target* — when careers and publications depend on clearing that bar — researchers (often unconsciously) run analyses until they find significance. This is **p-hacking**, which we examine in detail in [Chapter 11](lec11-multiple-testing.qmd), and it's a special case of **Goodhart's law**: the threshold was meant to *measure* evidence strength, and using it as a target for publishable results corrupts the measurement. [Chapter 16](lec16-feedback-loops.qmd) develops Goodhart's law in its general form — the same pattern that shows up in hospital readmission gaming, emissions cheating, and overfitting.
