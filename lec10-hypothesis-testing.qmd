...
In [Chapter 9](lec09-permutation-tests.qmd), we used a permutation test to build the null distribution — what the test statistic would look like if the drug did nothing — and computed a p-value. Today we formalize that machinery and discover its limitations.
...
1. **State $H_0$ (null hypothesis) and $H_1$ (alternative hypothesis)** — what are the competing claims?
...
| **Test statistic** | The **t-statistic**: difference in means ÷ standard error (formula below) |
...
::: {.callout-warning}
## The #1 p-value misconception (worth repeating)

[Chapter 9](lec09-permutation-tests.qmd) flagged this, but it bears repeating: the p-value is NOT the probability that $H_0$ is true. It's the probability of getting data at least as extreme as ours *if $H_0$ were true*. Big difference.
...
This p-value is so small that any conventional $\alpha$ rejects. But imagine a borderline case: a different trial produces p = 0.04. A colleague argues the result is "just barely significant" and shouldn't be trusted. Another says p < 0.05 means we reject. Who is right — and what does the disagreement reveal about the 0.05 threshold?
:::
...
- **Type I error** = convicting an innocent person. We rejected $H_0$ when it was actually true. In our clinical trial: we approve a useless drug for patients.
...
Let's visualize the tradeoff. Under the null, the test statistic is centered at zero. Under the alternative, it's shifted. The overlap is where errors happen. We use a smaller illustrative effect here so both error regions are visible; for the actual trial, the effect was large enough that Type II error was negligible. (The plot shows only the right tail of a two-sided test; the left tail is symmetric.)
...
# Use a small illustrative effect so both alpha and beta regions are visible
illustrative_effect = 1.0 * se
...
ax.plot(x, alt_dist, 'r-', lw=2, label=f'Alternative ($H_1$: effect = {illustrative_effect:.0f})')
...
**Power** is the probability of correctly rejecting $H_0$ when $H_1$ is true. Formally, power = $1 - \beta$, where $\beta$ is the Type II error rate. It depends on:
...
This fraction — the share of simulated experiments that correctly reject $H_0$ — is exactly $1 - \beta$.
...
Look at the power curves above. Roughly how many patients per group would you need to detect a 30 CD4 cell effect with 80% power? What about a 10 CD4 cell effect? (The next section computes the exact answer.)
:::

In practice, you'd use a **power analysis** tool rather than writing simulations from scratch. These tools take the effect size as **Cohen's d** — the effect measured in standard-deviation units ($d = \Delta / \sigma$), so that 0.2, 0.5, and 0.8 are conventionally called small, medium, and large effects regardless of the original units.

::: {.callout-note}
## Power analysis requires distributional assumptions

Power analysis requires specifying a model for the data. The `TTestIndPower` tool below assumes both groups are normally distributed with the same standard deviation. The required sample size is sensitive to these assumptions — if the true distribution is heavier-tailed or the variances differ substantially, the actual power may be lower than predicted.
:::
...
```{python}
# SE of the difference of means under the null (normal approximation, Chapter 8)
null_se = np.sqrt(control.var(ddof=1) / len(control) + treatment.var(ddof=1) / len(treatment))
print(f"SE of the difference of means: {null_se:.2f}")
```
...
The real effect is always there, but small samples often miss it. With n = 15 per group, a t-test detects the effect less than half the time — that's the power problem from the previous section. But the flip side is just as important:
...
A clinical trial with 10,000 participants tests a new blood pressure drug and finds a statistically significant reduction (p = 0.013). The actual effect is a 2 mmHg decrease in systolic blood pressure. For context, drinking a cup of coffee temporarily raises blood pressure by about 5 mmHg (Mesas et al., *Journal of Hypertension*, 2011). The drug's effect is real — it's not zero — but it is smaller than your morning coffee. Would you recommend this drug to patients, given the costs and potential side effects?
:::

Statistical significance told us the blood pressure effect is *not zero*. It said nothing about whether the effect is *worth acting on*. That judgment requires domain knowledge: what reduction is clinically meaningful, what are the side effects, and what alternatives exist.

:::{.callout-note collapse="true"}
## Going deeper: Goodhart's Law
...
We'll revisit these ideas in [Chapter 11](lec11-multiple-testing.qmd) when we ask: what happens when you run 20 tests at once? In [Chapter 12](lec12-regression-inference.qmd), we'll apply hypothesis tests to regression coefficients.
