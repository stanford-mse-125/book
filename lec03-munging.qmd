...
We'll use the left join going forward, so we don't lose any readmission data. The hospitals without matching info will simply have NaN for characteristics like ownership and star rating. (We'll see in Chapter 7 how missing values can be turned into useful *features*, and in Chapter 17 that tree-based models handle missing data natively, without requiring imputation.)
...
The broader lesson: always inspect dtypes before writing cleaning code. Applying string operations to a column that is already numeric will either fail or produce nonsense. The next example is trickier.
...
Where did those rows go? Comparing the state distribution before and after reveals that the drop is uneven.
