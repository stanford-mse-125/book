...
PC1 dominates. After PC1 the variance falls off quickly, but there is no sharp elbow — the tail is long and gradual. In finance this is the typical shape: one large factor (the market), a handful of mid-sized factors (sectors, styles), and a long tail of idiosyncratic structure.

:::{.callout-important}
## Definition: Elbow heuristic
A rough rule for choosing the number of components $k$ from a scree plot: keep components up to the point where the bar heights stop dropping quickly — the bend, or "elbow," in the plot. Components before the elbow capture meaningful structure; components after it look like noise. The elbow is a guide, not a prescription, and on real data (like the plot above) it can be ambiguous. The principled alternative is to choose $k$ by cross-validation against a downstream task — which is what we do in the payoff section below.
:::
...
In regression (Chapters 4 and 5) we projected $y$ onto a *given* subspace — the column space of the feature matrix $X$ — and measured how well we did by the squared length of the residual. PCA is the same kind of projection, except now we get to *choose* the subspace.

Each row $x_i$ of the standardized data matrix is a vector in $\mathbb{R}^p$ — one number per feature (one daily return per stock, in our case). A **$k$-dimensional subspace** $S \subset \mathbb{R}^p$ is the span of $k$ linearly independent vectors, just like the column space we worked with in regression. Given any such $S$, the projection $P_S x_i$ is the closest point in $S$ to $x_i$, and the squared distance from the data point to the subspace is $\|x_i - P_S x_i\|^2$.

PCA picks the subspace, not the projection.

:::{.callout-important}
## Definition: Principal Component Analysis (variational form)
Among all $k$-dimensional subspaces of $\mathbb{R}^p$, PCA picks the one closest to the data: the subspace $S^\star$ that minimizes the total squared distance from data points to the subspace,

$$S^\star = \arg\min_{\dim S = k}\; \sum_{i=1}^n \|x_i - P_S x_i\|^2.$$

Equivalently — using the Pythagorean identity $\|x_i\|^2 = \|P_S x_i\|^2 + \|x_i - P_S x_i\|^2$ — it is the subspace that maximizes the squared length of the projections, which (for mean-zero data) is the total variance captured.
:::

The **principal components** are an orthonormal basis for $S^\star$, ordered so that PC1 is the direction of greatest projected variance, PC2 the direction of greatest projected variance orthogonal to PC1, and so on. PC1, PC2, PC3 together span the optimal 3-dimensional subspace; PC1 through PC$k$ span the optimal $k$-dimensional subspace.

This is the variational characterization. It says *what* PCA is solving but not *how* to solve it. The answer is the **singular value decomposition** (SVD) of the data matrix.

:::{.callout-note collapse="true"}
## Going deeper: the SVD solves the variational problem

Every $n \times p$ matrix $X$ admits a decomposition
$$X = U S V^T,$$
where $U \in \mathbb{R}^{n \times r}$ and $V \in \mathbb{R}^{p \times r}$ have orthonormal columns and $S \in \mathbb{R}^{r \times r}$ is diagonal with nonnegative entries $\sigma_1 \geq \sigma_2 \geq \cdots \geq \sigma_r \geq 0$ (the **singular values**), where $r = \min(n,p)$.

The first $k$ columns of $V$, written $V_k$, form an orthonormal basis for the optimal subspace $S^\star$ in the variational problem above. The rank-$k$ truncation
$$X_k = U_k S_k V_k^T$$
is the closest rank-$k$ matrix to $X$ in squared Frobenius error. This last claim is the **Eckart-Young-Mirsky theorem** (Eckart & Young 1936, Mirsky 1960): no other rank-$k$ summary can do better.

The pieces of the SVD have direct interpretations:

- The columns of $V_k$ are the **principal component directions** — the orthonormal basis for the optimal subspace.
- The squared singular value $\sigma_i^2$ is proportional to the variance captured by PC$i$. The fraction $\sigma_i^2 / \sum_j \sigma_j^2$ is PC$i$'s **explained variance ratio**.
- The rows of $U_k S_k$ are the **PC scores**: each row is one data point's coordinates in the new basis.

For deeper SVD and PCA theory see Strang, *Introduction to Linear Algebra* (6th ed.) §7.4, or Hastie, Tibshirani, and Friedman, *The Elements of Statistical Learning* §14.5.
:::
...
When should we expect that? A common intuition is that real-world data are generated by a small number of unobserved drivers — latent factors that govern many observed features. Stock returns are the textbook example: each stock's daily return is approximately a linear combination of a market factor, a few sector factors, and idiosyncratic noise. Word counts in documents are driven by a few latent topics; face photographs by a few geometric factors (lighting, pose, expression); gene-expression profiles by a small number of cell types and developmental states. In every such setting the data matrix has a few large singular values and a long tail of small ones, and a low-rank PCA approximation captures most of the structure with a small $k$.

This latent-variable intuition is widely shared but the precise mathematics is subtler than it looks. Udell and Townsend (2019) prove a logarithmic-rank bound for matrices generated by smooth functions of bounded latent variables, but a 2024 follow-up by Budzinskiy in the same journal flags "several misconceptions" around the general claim and establishes the rigorous bound only for narrower function classes (inner-product, distance, and shift-invariant kernels). The story is the right intuition but it is one perspective among several.

For the running stock-return example a different formalization is sharper: the **approximate factor model** of econometrics. Bai (2003) and Stock and Watson (2002) show that when a panel of returns admits a decomposition into a few pervasive common factors plus weakly correlated idiosyncratic noise, PCA recovers those factors consistently as the panel grows. This is *the* mathematical justification for applying PCA to returns and is the body of work that underpins the eigenportfolio idea we use below.

If your data is *not* approximately low-rank — say, every feature is independent and identically distributed noise — then no rank-$k$ approximation will help. PCA still runs; it just has nothing to find. The scree plot tells you which regime you are in: a sharp drop after a few PCs means PCA will help, a flat scree means it will not.

## PCA as regression onto optimal covariates

The variational view picks the *subspace*. There is an equivalent picture that picks the *coordinates*, and it leads directly to factor models.

Once we have the principal directions $V_k$, each data point gets new coordinates in the basis they form — the PC scores. The reconstruction of the data matrix takes the form $X \approx F W^T$, with $F$ holding the scores (one row per data point) and $W = V_k$ holding the directions (one column per component). Equivalently, PCA jointly minimizes
$$\|X - F W^T\|_F^2$$
over both $F$ and $W$. Plain regression fixes $W$ and solves for $F$; PCA picks both — that is what "optimal covariates" means.
...
- Budzinskiy, S. (2024). "When big data actually are low-rank, or entrywise approximation of certain function-generated matrices." *SIAM Journal on Mathematics of Data Science*. Refines Udell & Townsend by identifying the specific function classes for which the dimension-independent rank bound holds.
- Bai, J. (2003). "Inferential theory for factor models of large dimensions." *Econometrica*, 71(1), 135-171. PCA estimates of the approximate factor model are consistent in large panels.
- Stock, J. H., and Watson, M. W. (2002). "Forecasting using principal components from a large number of predictors." *JASA*, 97(460), 1167-1179. Empirical/methodological foundation for using PCA on economic and financial panels.
