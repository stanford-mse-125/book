...
This chapter is about dimensionality reduction. The portfolios we construct are pedagogical, evaluated on historical data, with no transaction costs, no constraints on shorting, and no thought given to taxes. Real portfolio construction involves orders of magnitude more care. Treat the dollar numbers as an *evaluation rubric for our statistical method*, not as a recommendation about anything.
:::

A note on the spine: this is the one chapter where the textbook's Airbnb backbone is set aside. PCA's natural home is the high-dimensional, low-sample-count regime, and equities give us that without manufacturing it. We return to Airbnb in Chapter 15 (clustering).
...
This is the picture behind every PCA: **find the line (or plane, or subspace) closest to the data in perpendicular distance.** In two dimensions you can see it. In one hundred dimensions you cannot, but the math is identical.

:::{.callout-note collapse="true"}
## Going deeper: the original Pearson paper

Pearson's 1901 paper works the problem out in arbitrary dimensions using only Lagrange multipliers; the reformulation as an eigenvalue problem came later. The figure above is a modern reproduction of Pearson's geometric set-up, drawn with stock-return data instead of his original astronomical and biometric examples.
:::
...
So we have eleven years of daily returns for 95 S&P 100 constituents — the rest of the original 100 names had to be dropped for data-quality reasons (delistings, IPOs, missing days).

:::{.callout-warning}
## Survivorship bias
We use the *current* S&P 100 roster, frozen at the time the data were fetched. Companies that were in the index in 2014 but failed or were demoted before 2024 do not appear. This biases the dataset toward winners.

For pedagogical purposes the bias is harmless — the structure of returns we find is qualitatively right. For real risk research, use a survivorship-bias-free source like CRSP, which records the point-in-time index membership.
:::
...
print(f"PC1 explains {pca_raw.explained_variance_ratio_[0]*100:.1f}% of total variance")
print()
print("Top 5 stocks by absolute PC1 loading (no standardization):")
print(loadings_raw_pc1.abs().sort_values(ascending=False).head(5).round(3))
print()
print("Top 5 stocks by raw daily-return variance:")
print(returns.var().sort_values(ascending=False).head(5).map(lambda v: f"{v:.5f}"))
```

The top-PC1 stocks are largely the highest-volatility names — four of the top five overlap with the highest-raw-variance list. PC1 is not "the market" — PC1 is "TSLA and the other big movers." We have rediscovered which stocks have the largest raw variance. Useful, but it is not what we wanted.
...
print()
print(f"PC1 loadings — sign breakdown:")
print(f"  positive on {(loadings_pc1 > 0).sum()} / {len(loadings_pc1)} stocks")
print(f"  negative on {(loadings_pc1 < 0).sum()} / {len(loadings_pc1)} stocks")
print(f"  mean loading: {loadings_pc1.mean():+.3f}")
```

After standardization the picture flips completely. PC1 loads positively on essentially every stock in the universe. It is a single direction in which the entire S&P 100 moves up together (or down together). **This is the market factor.** It was always there in the data; raw-scale PCA could not see it.
...
PC3 turns out to be the growth-versus-yield contrast: Nvidia, Amazon, Meta, Adobe, Alphabet on one side; utilities (Duke, Southern), staples (Coca-Cola, Altria), and high-dividend telecom (Verizon) on the other. By PC5 or PC6, the loadings start looking arbitrary — the components explain real variance, but the economic story runs out.
...
A one-paragraph caveat. PCA factor models are a starting point. Production risk models (Barra, Axioma, the academic Fama-French and Carhart factors) add fundamentals, industry classifications, and time-varying volatility, and they handle the rebalance-cost and turnover concerns we ignored here. The PCA factor model gives the simplest version of the right idea; everything beyond it is engineering.
