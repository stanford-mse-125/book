...
Let's see what happens if we run PCA on the raw (unstandardized) data. This is what an AI assistant would typically do.

**Think about it:** Before looking at the output — which feature do you think PC1 will pick up, and why?
...
**There it is.** `maximum_nights` has a variance of millions because some listings allow stays of 1,000+ nights. `availability_365` ranges up to 365. Meanwhile, `bathrooms` ranges from 0 to 10.
...
This is a critical lesson: **PCA is dominated by scale.**
...
The **scree plot** shows how much variance each PC explains. We look for an **elbow** — the point where adding more PCs doesn't help much. This is called the **elbow method**.
...
**Think about it:** Now that we've standardized, what groupings of features do you expect PCA to find? Think about which Airbnb features tend to be correlated with each other.
...
- **PC1** loads heavily on the review scores (accuracy, cleanliness, communication, value, rating). This is a "review quality" factor — listings that score high on PC1 have uniformly good reviews.

- **PC2** loads on size features (accommodates, bedrooms, beds, bathrooms) and price. This is a "size/luxury" factor — bigger, pricier listings score higher on PC2.

Notice how PCA automatically discovered these natural groupings. We didn't tell it about "review quality" or "listing size" — it found these dimensions because they capture the most variance.

### Tracing one listing through PCA

Let's pick one listing and trace it through the entire PCA pipeline. This makes the loadings concrete.
...
Now that you've seen PCA work, let's look under the hood. PCA is powered by the **SVD** (**Singular Value Decomposition**). If you've taken EE103, see VMLS Chapter 16 for a deeper treatment.

The exact SVD of the centered data matrix is:
...
- The columns of $US$ are the **PC scores** (each listing's coordinates in the new basis)

The **Eckart-Young-Mirsky theorem** guarantees this: among all rank-$k$ approximations to $X$, the truncated SVD has the smallest total squared error. No other $k$-dimensional summary can do better.
...
PCA is the same optimization, except we also get to choose $X$. We're minimizing $\|Y - XW\|_F^2$ over *both* the scores $X$ and the directions $W$ simultaneously. PCA finds the best covariates AND the best coefficients.

These two views — maximizing variance captured and minimizing reconstruction error — are mathematically equivalent. This is why PCA feels like a natural extension of regression: it IS regression, but with the freedom to pick the best possible features.
...
## The surprise: standardization changes everything
...
**Completely different!** Without standardization, PC1 is dominated by `maximum_nights` and `availability_365` — just because those columns have big numbers. With standardization, PC1 captures the review quality pattern we identified earlier.

This is one of the most common PCA mistakes in practice. Always check whether your data has been standardized.

**Rule of thumb:** always standardize before PCA, unless you have a specific reason not to (e.g., all features are already on the same scale).

## PCA vs. feature selection

**Think about it:** Couldn't we just pick the 5 most important features instead of doing PCA? When would PCA be better than, say, Lasso (Lecture 7)?

PCA creates *new* features that are combinations of the originals — it's **dimensionality reduction**. Lasso picks a subset of existing features — that's **feature selection**. PCA is especially useful when your original features are correlated (like the six review scores), because it creates uncorrelated summaries that capture more information per component.

## A caution: PCA on mixed data
...
- **SVD (Singular Value Decomposition)**: the matrix factorization $X = USV^T$ that powers PCA. The columns of $V$ are the PC directions, the diagonal of $S$ holds the singular values, and $US$ gives the PC scores.
...
1. PCA finds the best low-dimensional linear summary of your data by maximizing captured variance (equivalently, minimizing reconstruction error).
2. PCA is regression where you also get to choose the features — it finds the optimal covariates AND coefficients simultaneously.
3. Always standardize before PCA, or the result is dominated by whichever column has the biggest numbers.
4. The Eckart-Young-Mirsky theorem guarantees that the SVD gives the best rank-$k$ approximation — no other $k$-dimensional summary can do better.
5. PCA creates new combined features (dimensionality reduction), while Lasso selects a subset of existing features (feature selection).
