...
In **Chapter 15**, we'll move from summarizing data to *grouping* data. **K-means clustering** will discover modern NBA archetypes that cut across traditional positions — and then we'll apply the same algorithm to a criminal-risk dataset where the feature choice matters far more than the basketball one. PCA and K-means are cousins: same optimization framework, different constraints.
