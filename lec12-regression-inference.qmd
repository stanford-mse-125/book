...
- The test statistic is labeled **z** instead of **t**. Logistic regression uses the **Wald test** (asymptotically normal) rather than the exact t-test. For large $n$, the two agree.
...
The table has the same structure as the OLS output from earlier in this chapter, with z-statistics in place of t-statistics.
...
Because the features are standardized, each odds ratio is the multiplicative change in odds per **one-SD** increase in that predictor, holding the others constant. To convert to per-unit (say, per year for age), divide the coefficient by the feature's SD before exponentiating.
...
- **Wald test** (for logistic regression coefficients): asymptotically-normal analog of the t-test; the z-statistic $\hat{\beta}_j / \text{SE}(\hat{\beta}_j)$ is compared to a standard normal. CIs on $\hat{\beta}_j$ map to CIs on the odds ratio $e^{\hat{\beta}_j}$ by exponentiating.
