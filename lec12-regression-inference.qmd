...
Under $H_0$, this statistic follows a $t$-distribution with $n - p - 1$ degrees of freedom, where $n$ is the number of observations and $p$ is the number of predictors. The "$-p-1$" accounts for the parameters we have already estimated: one coefficient per predictor, plus the intercept.
:::
...
Here $t^*$ is the **critical value** from the $t$-distribution with $n - p - 1$ degrees of freedom at the desired confidence level. For a 95% CI, $t^*$ is the 97.5th percentile of that distribution (so that 95% of the distribution lies between $-t^*$ and $t^*$). When $n$ is large, $t^* \approx 1.96$, the familiar normal critical value.
...
The x-axis of the residual plot shows the **fitted values** -- the model's predicted response for each observation, $\hat{y}_i$. The orange curve is a **LOWESS smooth** (locally weighted scatterplot smoothing): a flexible local average that highlights any systematic trend in the residuals. A flat LOWESS line near zero suggests the model's assumptions are met; a bend or funnel shape signals a problem.
...
The NBA Q-Q plot above shows heavy tails — but what do other departures from normality look like? The gallery below pairs each residual distribution (top row of each block) with its Q-Q plot (bottom row), so you can read directly from distribution shape to Q-Q shape.
...
The table above is a standard OLS coefficient table from `statsmodels`. Every regression output you encounter — in Python, R, Stata, or a journal article — contains the same elements. Let's walk through each column.

```{python}
# Print the full model summary for reference
print(airbnb_model.summary())
```

Here is what each section of the output means:

1. **`coef` (Coefficients)**: The estimated $\hat{\beta}_j$ for each predictor. Each coefficient answers: "holding everything else constant, how much does the predicted price change for a one-unit increase in this variable?" For example, the `bathrooms` coefficient means each additional bathroom is associated with roughly \$62 more per night, after controlling for bedrooms and borough.

2. **`std err` (Standard Error)**: The estimated standard deviation of $\hat{\beta}_j$ across hypothetical repeated samples. A smaller standard error means the estimate is more precise. The standard error for `bathrooms` is about \$1.86 — small relative to the coefficient, so the estimate is fairly precise.

3. **`t` (t-statistic)**: The ratio $\hat{\beta}_j / \text{SE}(\hat{\beta}_j)$. A large absolute value (typically $|t| > 2$) suggests the coefficient is distinguishable from zero. Values near zero mean the predictor may not contribute to the model.

4. **`P>|t|` (p-value)**: The probability of observing a t-statistic this extreme if $\beta_j = 0$. Small p-values (typically $< 0.05$) lead us to reject the null that the coefficient is zero. But remember: a small p-value does not guarantee a large or important effect.

5. **`[0.025, 0.975]` (95% Confidence Interval)**: The range of plausible values for $\beta_j$. If this interval does not contain zero, the coefficient is statistically significant at the 5% level. The width of the interval tells you how precisely the effect is estimated.

6. **`R-squared`** (top of the output): The fraction of variance in the response explained by the model. An $R^2$ of 0.40 means the predictors explain 40% of the variation in price; the remaining 60% is unexplained. **Adj. R-squared** penalizes for the number of predictors — useful when comparing models of different sizes.

7. **`F-statistic`** (top of the output): Tests whether the model as a whole explains more variance than a model with no predictors (intercept only). A large F-statistic with a small p-value (Prob (F-statistic)) means at least one predictor is useful. Think of it as an "overall significance" test for the entire regression.

8. **`Cond. No.` (Condition Number)**: A measure of multicollinearity among predictors. Values above 30 suggest moderate multicollinearity; values above 1,000 signal that some predictors are nearly linearly dependent, which inflates standard errors and makes individual coefficients hard to interpret.

```{python}
# Summarize the key numbers for quick reference
print("Quick summary of model diagnostics:")
print(f"  R-squared:        {airbnb_model.rsquared:.3f}")
print(f"  Adj. R-squared:   {airbnb_model.rsquared_adj:.3f}")
print(f"  F-statistic:      {airbnb_model.fvalue:.1f}  (p = {format_pvalue(airbnb_model.f_pvalue)})")
print(f"  Condition number: {airbnb_model.condition_number:.0f}")
print()
if airbnb_model.condition_number > 1000:
    print("  ⚠ High condition number — check for multicollinearity.")
elif airbnb_model.condition_number > 30:
    print("  Moderate condition number — some correlation among predictors, but manageable.")
else:
    print("  Condition number is low — no multicollinearity concern.")
```
...
The same t-test and confidence interval machinery from linear regression applies to logistic regression coefficients — but the interpretation changes. In linear regression, a coefficient tells you how many units the response changes per unit increase in the predictor. In logistic regression, the response is a log-odds, so the coefficient tells you how the **log-odds** change — and exponentiating gives you an **odds ratio**.

We demonstrate with the **Framingham Heart Study**, a landmark longitudinal study that followed thousands of residents of Framingham, Massachusetts to identify risk factors for cardiovascular disease. The outcome is `TenYearCHD`: whether the patient developed coronary heart disease within ten years.

```{python}
# Load and prepare Framingham data
framingham = pd.read_csv(f'{DATA_DIR}/framingham/framingham.csv')
print(f"Observations: {len(framingham):,}")
print(f"Missing values per column:")
print(framingham.isnull().sum()[framingham.isnull().sum() > 0].to_string())
print(f"\nOutcome (TenYearCHD): {framingham['TenYearCHD'].mean():.1%} developed CHD")
```

```{python}
# Drop rows with missing values for a clean analysis
fram = framingham.dropna().copy()
print(f"Complete cases: {len(fram):,} (dropped {len(framingham) - len(fram):,} with missing values)")
```

We fit a logistic regression using **statsmodels** to get the full inference table — coefficients, standard errors, test statistics, p-values, and confidence intervals. We use a subset of clinically meaningful predictors.

```{python}
# Fit logistic regression with statsmodels
y = fram['TenYearCHD']
X = fram[['age', 'male', 'BMI', 'sysBP', 'cigsPerDay', 'glucose']]
X = sm.add_constant(X)  # add intercept

logit_model = sm.Logit(y, X).fit(disp=0)  # disp=0 suppresses optimization output
print(logit_model.summary())
```

The output looks similar to the OLS tables we have seen, with one key difference: the test statistic column is labeled **z** instead of **t**. Logistic regression uses the **Wald test** (asymptotically normal) rather than the t-test (exact under normality). For large samples, the distinction is minor — both compare the coefficient to its standard error.

### Odds ratios: interpreting logistic regression coefficients

In linear regression, $\hat{\beta}_j$ is a change in $y$. In logistic regression, $\hat{\beta}_j$ is a change in the **log-odds** of the outcome. Exponentiating converts this to an **odds ratio**:

$$\text{Odds Ratio}_j = e^{\hat{\beta}_j}$$

:::{.callout-important}
## Definition: Odds Ratio
The **odds ratio** $e^{\hat{\beta}_j}$ tells you how the odds of the outcome multiply for each one-unit increase in $x_j$, holding all other predictors constant. An odds ratio of 1.5 means the odds increase by 50%; an odds ratio of 0.8 means the odds decrease by 20%.
:::
...
coef_table['Odds Ratio'] = np.exp(logit_model.params)
coef_table['OR 95% CI Lower'] = np.exp(ci[0])
coef_table['OR 95% CI Upper'] = np.exp(ci[1])

# Display without the intercept (odds ratio for the intercept is not meaningful)
display_cols = ['Odds Ratio', 'OR 95% CI Lower', 'OR 95% CI Upper', 'p-value']
print(coef_table.loc[coef_table.index != 'const', display_cols].round(4).to_string())
```

```{python}
# Interpret the key predictors
or_age = np.exp(logit_model.params['age'])
or_bmi = np.exp(logit_model.params['BMI'])
or_sysbp = np.exp(logit_model.params['sysBP'])
ci_age = np.exp(logit_model.conf_int().loc['age'])

print("Interpreting odds ratios:")
print(f"  age:     OR = {or_age:.3f}  ->  each additional year of age multiplies")
print(f"           the odds of CHD by {or_age:.3f} (95% CI: [{ci_age[0]:.3f}, {ci_age[1]:.3f}])")
print(f"  BMI:     OR = {or_bmi:.3f}  ->  each unit increase in BMI multiplies the odds by {or_bmi:.3f}")
print(f"  sysBP:   OR = {or_sysbp:.4f} ->  each mmHg increase in systolic BP multiplies the odds by {or_sysbp:.4f}")
print()
print("Odds ratios near 1 indicate little effect; far from 1 indicates a strong association.")
print("This is the standard language of epidemiology and policy research.")
```

```{python}
#| code-fold: true
# Visualize odds ratios with CIs (forest plot)
fig, ax = plt.subplots(figsize=(8, 5))

predictors = [p for p in logit_model.params.index if p != 'const']
ors = np.exp(logit_model.params[predictors])
or_ci_lower = np.exp(logit_model.conf_int().loc[predictors, 0])
or_ci_upper = np.exp(logit_model.conf_int().loc[predictors, 1])
...
ax.set_title('Odds Ratios for 10-Year CHD Risk\n(Framingham Heart Study)')
ax.legend()
plt.tight_layout()
plt.show()

print("An odds ratio whose CI excludes 1 is statistically significant.")
print("An odds ratio whose CI includes 1 may not be distinguishable from 'no effect.'")
```

:::{.callout-tip}
## Think About It
The coefficient on `age` is small (a fraction), but the odds ratio compounds over years. A 10-year age difference multiplies the odds by $e^{10 \times \hat{\beta}_{\text{age}}}$. How large is that for the Framingham model?
:::

:::{.callout-note}
## CIs on odds ratios
To build a confidence interval for the odds ratio, exponentiate the CI endpoints for $\hat{\beta}_j$. Because the exponential function is monotone increasing, a 95% CI for $\beta_j$ of $[a, b]$ maps to a 95% CI for $e^{\beta_j}$ of $[e^a, e^b]$. This is exactly what the table above displays.
:::


## Calibration: do predicted probabilities mean what they say?

A logistic regression predicts probabilities, not just class labels. But do those probabilities mean what they claim? If a model predicts a 70% probability of heart disease for a group of patients, did roughly 70% of them actually develop CHD?
...
axes[0].plot(prob_pred, prob_true, 'o-', color='steelblue', markersize=8, label='Logistic regression')
...
axes[1].hist(y_prob, bins=30, color='steelblue', edgecolor='black', alpha=0.7)
axes[1].set_xlabel('Predicted Probability of CHD')
...
print("Left: points near the diagonal indicate good calibration.")
print("Right: most predictions cluster at low probabilities (CHD is relatively rare).")
```

```{python}
# Quantify calibration: for each bin, compare predicted vs observed
print("Calibration check (per bin):")
print(f"  {'Predicted':>12s}  {'Observed':>12s}  {'Difference':>12s}")
for p_pred, p_true in zip(prob_pred, prob_true):
    print(f"  {p_pred:12.3f}  {p_true:12.3f}  {p_true - p_pred:+12.3f}")
```

### Why calibration matters for decisions

Calibration directly affects decision-making. Suppose a doctor uses a model-predicted risk threshold of 30% to recommend preventive treatment. If the model systematically overestimates risk — predicting 30% when the true rate is 15% — patients receive unnecessary interventions. If it underestimates, patients who need treatment are missed.

For linear regression, we check whether the model's assumptions hold using residual plots and Q-Q plots. For classification, **calibration** fills an analogous role: it checks whether the model's probability outputs are trustworthy. A model can have high accuracy or AUC yet be poorly calibrated — accurate in ranking patients by risk, but wrong about the absolute risk level.

:::{.callout-tip}
## Think About It
A model with good AUC but poor calibration ranks observations correctly (high risk above low risk) but gets the absolute probabilities wrong. When would ranking be sufficient? When would you need well-calibrated probabilities?
:::

**Next up:** [Chapter 12.5](lec12.5-classification-inference.qmd) applies everything
from Act 2 to classification problems. Then [Chapter 13](lec13-trees.qmd) opens Act 3
with a completely different model family: decision trees and random forests.
...
- **Odds ratio**: $e^{\hat{\beta}_j}$ — how the odds of the outcome multiply per one-unit increase in $x_j$, holding other predictors constant. Confidence intervals on $\hat{\beta}_j$ map to CIs on the odds ratio by exponentiating.
- **Calibration**: A model is well-calibrated if predicted probabilities match observed frequencies. Check with a calibration plot (observed fraction of positives vs. mean predicted probability per bin).
...
- `calibration_curve(y_true, y_prob, n_bins=10)` -- compute observed frequency vs. predicted probability for calibration plots
...
- Know that $e^{\hat{\beta}_j}$ is an odds ratio and be able to interpret one in context (e.g., "each additional year of age multiplies the odds of CHD by 1.05").
- Know what a calibration plot shows and why good calibration matters for decisions.
