...
But now consider a different question. An Airbnb host is thinking about spending \$50,000 to convert a closet into a second bedroom. The regression coefficient on `bedrooms` is +\$85/night. Does that mean the renovation will pay for itself in two years?

Not necessarily. The coefficient tells you that listings *with* more bedrooms tend to cost more — but it doesn't tell you that *adding* a bedroom to *your* listing would increase your price by \$85. Maybe the real driver is apartment size, or building quality, or neighborhood. Larger apartments happen to have more bedrooms *and* higher prices, but the extra bedroom isn't doing the work.
...
drop it without loss?** Parsimony matters -- the best model is the simplest one that
...
hold everything else constant in the real world. In a more advanced course, you may see the same regression coefficients interpreted from three perspectives: prediction, inference, and causality. We explore the distinction in Lecture 18.
...
print()
print("The ratio is about 2 -- the coefficient sits roughly 2 SEs from zero.")
```
...
sample size, $n > 100{,}000$. With smaller samples, you would want to check a residual Q-Q plot.)
...
### What LINE looks like: satisfied vs. violated

Before examining our NBA residuals, it helps to see what each LINE condition looks like
when it holds and when it fails. The following synthetic examples illustrate four
diagnostic scenarios in a single panel.
...
- **Bootstrap confidence intervals** (Lecture 8) do not assume normality -- they let
...
:::{.callout-tip}
## Think About It
The t-test says REST_DAYS is "statistically significant" ($p \approx 0.04$).
...
Let us return to the NBA model. The REST_DAYS coefficient was statistically significant ($p \approx 0.04$).
...
with $p = 0.04$!" What questions should you ask?
:::
...
We formalize this distinction in Lecture 18 using causal DAGs. For now, remember:
...
Before modeling, let us visualize. The `neighbourhood_group_cleansed` column represents
...
grid = pd.DataFrame({
    ...
    'neighbourhood_group_cleansed': ['Manhattan'] * len(bath_vals),
})
...
We revisit this topic in Lecture 18 when we ask: coaches *choose* when to rest players --
...
- Residual plots and Q-Q plots check whether the model's assumptions (LINE conditions) hold; when they do not, use bootstrap CIs (Lecture 8) instead of formula CIs.
