...
:::{.callout-tip}
## Think About It
Before looking at the numbers, which variable do you think has the
...
We've built confidence intervals for **coefficients** -- but what about predictions?
When we use a regression model to predict at a particular value of $x$, there are
two different questions we might ask, and they give different intervals.

- **Confidence interval for the mean response**: How uncertain are we about the
  *average* $Y$ at this $x$? This reflects estimation uncertainty in $\hat{\beta}$.
- **Prediction interval for a new observation**: Where might a *single new* $Y$ fall?
  This adds individual variability ($\varepsilon$) on top of estimation uncertainty.

:::{.callout-important}
## Prediction intervals are always wider
A prediction interval accounts for **two sources of uncertainty**: (1) we don't know
the true regression line exactly (estimation uncertainty), and (2) individual
observations scatter around the line (residual variability $\sigma^2$). A confidence
interval for the mean only includes source (1).
:::

```{python}
# Prediction interval vs confidence interval — Airbnb model
# get_prediction returns both CI for the mean and PI for a new observation
pred_data = airbnb_clean.copy()
predictions = airbnb_model.get_prediction(pred_data)
pred_summary = predictions.summary_frame(alpha=0.05)

# Pick one predictor to plot against: bathrooms
# Average across other covariates by plotting prediction vs bathrooms
...
print("The blue band (CI for the mean) is narrow: we're fairly sure about the average price.")
print("The orange band (prediction interval) is wide: individual listings vary a lot around that average.")
```
...
:::{.callout-tip}
## Think About It
A sports analytics team comes to you and says "We've proven rest helps
...
- **Prediction interval**: An interval for where a *single new* observation might fall at a given $x$. Always wider than the confidence interval for the mean, because it includes both estimation uncertainty and individual variability.
...
- Residual plots and Q-Q plots check whether the model's assumptions (LINE conditions) hold; when they don't, use bootstrap CIs (Lecture 8) instead of formula CIs.
