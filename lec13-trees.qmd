...
A decision tree is a flowchart drawn upside down:

- **Root** — the topmost node, where the first split happens.
- **Internal node** — any node that asks a yes/no question and routes the observation to one of its two children.
- **Leaf** — a terminal node where splitting stops. Every leaf carries a prediction (a price for regression, a class label for classification).
- **Depth** — the length of the longest root-to-leaf path. Controlled by `max_depth`.
:::
...
plot_tree(tree_viz, feature_names=feature_names, filled=True, fontsize=14,
          rounded=True, ax=ax, impurity=False, proportion=False)
ax.set_title('Decision tree for Airbnb price prediction (depth=3)', fontsize=18)
plt.tight_layout()
plt.show()
```

Each leaf shows the predicted price and the sample count. The tree discovers that room type and borough matter without any one-hot encoding intuition. (Sklearn's trees need numeric input, so we one-hot encoded; CART/XGBoost/LightGBM split on raw categories directly.)


### How a decision tree splits

A decision tree partitions the feature space using a **greedy recursive** algorithm. At each node, it picks a feature $j$ and threshold $s$ to minimize total squared error in the two child regions:

$$\min_{j,\, s} \left[ \sum_{x_i \in R_1(j,s)} (y_i - \bar{y}_1)^2 + \sum_{x_i \in R_2(j,s)} (y_i - \bar{y}_2)^2 \right]$$

where each leaf predicts the mean $\bar{y}$ of its training observations. The algorithm enumerates every candidate (feature, threshold) pair (for `bedrooms`: thresholds at 0.5, 1.5, 2.5, …) and picks the one that drops SSE most. The same scan runs again inside each child until a stopping criterion (max depth, minimum leaf size) kicks in.

For classification, the splitting criterion swaps to **Gini impurity** — a measure of how mixed each child is, formally defined in §Classification trees below.

#### A worked example on a tiny dataset

Easier to *see* recursive partitioning than to read it. A 2-feature classification toy with 22 points — class-0 strip along the bottom, class-0 cluster upper-left, class-1 cluster upper-right. Fit a depth-2 tree and watch it split.
...
def scatter_pts(ax):
    ax.scatter(toy_X[toy_y==0, 0], toy_X[toy_y==0, 1], s=80, c='#fde68a',
               edgecolors='black', linewidth=1, label='class 0')
    ax.scatter(toy_X[toy_y==1, 0], toy_X[toy_y==1, 1], s=80, c='#86efac',
...
The first split (`x_2 ≤ 1.95`) peels the bottom strip off — that child is pure. The upper child still mixes classes, so it gets a second split (`x_1 ≤ 2.00`) separating the upper-left class-0 cluster from the upper-right class-1 cluster. Three pure leaves.
...
A **random forest** fits hundreds of decision trees, each on a random subset of the data and features, and averages their predictions. The randomness ensures trees disagree on noise but agree on signal.
...
The forest beats the single tree. `n_estimators=100` is the number of trees; `min_samples_leaf=5` caps each tree's depth near $\log_2(n/5)$, deep enough to capture structure but not so deep that a leaf depends on two or three quirky bootstrap rows.
...
The ensemble prediction averages tree predictions (regression) or takes majority vote (classification). Each tree memorizes *different* noise from its own bootstrap sample, so averaging cancels it.
...
A random forest is one **ensemble**: many weaker learners aggregated into a stronger predictor. Two strategies dominate:
...
# Vertical stack: panels share the bedrooms x-axis.
...
The top panel shows 25 deep trees and their thick-line average. Individual trees wiggle; the average is steadier. The bottom panel makes "steadier" precise: each curve is the standard deviation of the prediction across resampled forests of size $B$. A single tree swings by tens of dollars across bootstrap samples; each $4\times$ more trees roughly halves the spread, and the curves bend toward a floor as the trees become correlated.

That floor is the punchline. If two trees almost always make the same mistake, averaging them barely helps; if they make *independent* mistakes, averaging cancels the noise. **Feature subsampling** decorrelates trees by forcing different splits, so the variance floor drops.
...
Averaging shrinks the *spread* of the prediction without pulling its center anywhere — the bottom panel curves shrink vertically as $B$ grows, but none drift left or right. A random forest keeps each tree deep (low bias) and reduces variance by *averaging* — sidestepping the complexity-tuning dance, replacing the U-curve in `max_depth` with a one-sided staircase in `n_estimators`.
...
The random forest wins with zero feature engineering — no polynomial terms, no interactions, no decisions about which features to cross. For tabular prediction, a forest is a strong default benchmark: if your hand-crafted model can't beat it, the engineering effort may not be worthwhile.
:::
...
Same recipe for classification: bag deep trees, average their probability estimates. Swap MSE for log-loss; evaluate with AUC.
...
**Native missing-value handling.** XGBoost, LightGBM, and (since scikit-learn 1.3) sklearn's `DecisionTreeRegressor`/`Classifier` learn at each split which child missing-valued rows go to, picking whichever direction reduces the loss more. No imputation needed.

**Surrogate splits** (the original CART approach) instead route missing values via a backup feature with a similar partition. Not implemented in sklearn.

:::{.callout-note collapse="true"}
## Going deeper: XGBoost vs. LightGBM

Both fit gradient-boosted trees, both handle missing values natively, both win Kaggle leaderboards. They differ in *how* they grow each tree.
...
Forests look strong on tabular benchmarks, but "the forest wins, ship it" hides three failure modes:

- **Distribution shift.** A forest predicts by interpolating between training leaves. A listing in a region the forest never saw — a new neighborhood, a post-pandemic price regime, a new apartment type — gets a prediction that is fundamentally an interpolation between unrelated training points. [Chapter 6](lec06-validation.qmd)'s distribution-shift point applies here too.

- **Feature importance is descriptive, not causal.** High MDI on `accommodates` doesn't mean adding capacity raises price; it means the forest splits on `accommodates` often, because that variable correlates with neighborhood, room type, and season the forest hasn't separated. Same caveat for permutation importance.

- **Correlated features share their importance.** If `bedrooms` and `accommodates` carry overlapping signal, dropping either barely hurts — each one's importance is split between them. Name the *group*, not a single winner.

The forest is a strong default benchmark, not a finished answer. Trust it on inputs that look like training data, distrust the importance chart as a causal claim.

:::{.callout-tip}
## Back to the host

Friday's prototype: ship the forest. It beats your hand-engineered linear model with no manual feature crosses, and it handles missing rooms, boroughs, and review counts without imputation choices that bias the answer. Set up drift monitoring on the inputs — a forest blind-extrapolates into new neighborhoods or new price regimes. And don't sell the importance chart as causal: it tells the host *what the model used*, not *what raises a price*. Wisdom of the crowd, even of overfit trees.
:::
...
- **Random forests average overfit trees, and the average is stable.** Bagging plus feature subsampling decorrelates the trees, so averaging them cancels the noise without raising bias. Adding more trees rarely hurts — no U-curve in `n_estimators`.
