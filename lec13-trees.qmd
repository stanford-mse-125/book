...
Concretely, picking the very first split looks like this. The algorithm enumerates every candidate (feature, threshold) pair: `bedrooms ≤ 0.5`, `bedrooms ≤ 1.5`, …, `bathrooms ≤ 1.5`, `room_type = Entire home`, and so on. (For a numeric feature, sklearn's candidate thresholds are midpoints between consecutive sorted values, so an integer feature like `bedrooms` produces thresholds at 0.5, 1.5, 2.5, …) For each candidate it would compute the total squared error in the resulting two child nodes — predicted by the mean price of listings on each side — and the candidate that drops total SSE the most wins. If `bedrooms ≤ 1.5` wins, studios and 1-bedrooms go left (predicted at their mean price); 2-bedrooms and up go right (predicted at theirs). The same scan then runs again inside each child node.
...
For classification, the only thing that changes is the splitting criterion: instead of squared error, the tree uses **Gini impurity** to score how mixed each child node is. We come back to it in §Classification trees below.
...
Let's visualize the predictions from a shallow and deep tree to see what overfitting looks like. Bedrooms is integer-valued in the data, so we plot the tree's prediction at each integer bedroom count, holding the other features fixed at their training-set medians.

```{python}
# Compare shallow vs deep tree predictions at each integer bedroom count
fig, axes = plt.subplots(1, 2, figsize=(14, 5), sharey=True)

# Prediction grid: integer bedrooms only, other features at median
median_features = X_train.median()
bedroom_range = np.arange(0, 7)
...
for ax, depth, title in zip(axes,
    ...
    ax.plot(bedroom_range, preds, 'o', color='orangered', markersize=10,
            label=f'depth={depth}')
    ...
    ax.legend()
    ax.set_xlim(-0.5, 6.5)
    ax.set_xticks(bedroom_range)

plt.tight_layout()
plt.show()
```

The shallow tree's predictions march smoothly upward with bedroom count; the deep tree's predictions jump erratically — 3-bedroom listings are predicted *cheaper* than 2-bedroom listings, an artifact of memorizing a few unusual rows in the training set rather than learning the real pattern.

**Why constant predictions, not a smooth curve?** A tree partitions the feature space into axis-aligned rectangles. Each rectangle (a leaf) gets a single number — the mean of the training observations that fall into it. As you slide along any one feature, the prediction stays flat until you cross a split threshold, then jumps to a new constant. Linear regression draws a continuous slope; trees produce predictions that are constant within neighborhoods of feature space. The deep tree carves out so many tiny rectangles that adjacent integer bedroom counts can land in different leaves with very different means, which is why its predictions look erratic instead of orderly.
...
Squared error doesn't make sense when the target is a label rather than a number. We need a different way to ask "how mixed is this node?"

The **Gini index** for a binary classification node with class proportion $\hat p$ is

$$\text{Gini} = \hat p (1 - \hat p).$$

Read it as a measure of impurity:
- A **pure** node — all observations have the same label, so $\hat p = 0$ or $\hat p = 1$ — has Gini $= 0$.
- A **maximally mixed** node — 50/50 — has Gini $= 0.25$, the largest value possible for binary classification.

*Worked example.* A node containing 70 expensive listings and 30 not-expensive listings has $\hat p = 0.7$, so Gini $= 0.7 \times 0.3 = 0.21$. A second candidate split that produced one child with 90/10 (Gini $= 0.09$) and another with 50/50 (Gini $= 0.25$) would average to a weighted impurity that the algorithm compares against the parent's Gini to decide whether the split is worth taking.

The multi-class version generalizes the same idea: $\text{Gini} = 1 - \sum_k \hat p_k^2$.

The tree-fitting algorithm greedily picks splits that maximize the **decrease** in total weighted Gini from parent to children — the categorical analogue of "drop SSE the most" from regression.
:::
...
- **Bagging** (parallel) — what we just did. Train many learners independently, each on a different bootstrap sample of the data; combine by averaging or majority vote. The mechanism is *variance reduction*. Random forests = bagging trees + feature subsampling, so the trees disagree on noise as much as possible.

- **Boosting** (sequential). Train learners one at a time, each focused on the previous ones' mistakes; combine as a weighted sum. The mechanism is *bias reduction* — each new learner peels off another layer of error. *Gradient boosting* — the variant that fits each new learner to the residuals of the current ensemble — is treated in [Chapter 17](lec17-automl-llms.qmd).

The general lesson: ensembling is a recipe for turning a moderately good model into a much better one without hand-engineering features. It is the reason "use a random forest" is the strong default for tabular data, and the reason gradient-boosted trees win much of the Kaggle leaderboard.
...
- **Ensemble** — a model built by aggregating many weaker learners. Bagging (parallel, e.g. random forests) reduces variance; boosting (sequential, e.g. gradient boosting in Chapter 17) reduces bias.
