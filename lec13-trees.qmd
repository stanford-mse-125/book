...
Everything above predicts a continuous number (price). **Classification** predicts a discrete category instead — "expensive or not," "spam or not." You already know logistic regression from [Chapter 7](lec07-classification.qmd); trees handle classification just as naturally as regression. A classification tree predicts the majority class in each leaf, and a random forest classifier uses majority vote across trees.
::::
