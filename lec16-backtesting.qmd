---
title: "Lecture 16: Backtesting and Time Series Validation"
execute:
  enabled: true
jupyter: python3
---

You build a model to predict tomorrow's air quality. It scores R² = 0.85 on your test set. You deploy it. It's terrible. *What happened?*

The problem isn't your model — it's how you evaluated it. When data has a **temporal structure**, the standard random train/test split silently lies to you. Today we'll see exactly how, quantify the damage, and learn the right way to validate time series models.
...
This is **non-stationary** data — the mean and variance change over time, and extreme events (wildfire spikes) appear unpredictably. This is a concept we haven't seen before in the course; we'll explore why it matters throughout this lecture.

## Building a prediction model

Let's try to predict tomorrow's AQI. A natural approach: use **lag features** — yesterday's AQI, the 7-day average, etc. This is just the feature engineering from Lecture 6, applied to time.
...
## The naive mistake: random train/test split

In Lecture 7, we used random train/test splits and k-fold cross-validation. That worked because the data had no temporal structure. Here, it doesn't.
...
```{python}
# Random 80/20 split -- THE NAIVE APPROACH
np.random.seed(42)
...
model_naive.fit(train_naive[features], train_naive['AQI'])

pred_naive = model_naive.predict(test_naive[features])
r2_naive = r2_score(test_naive['AQI'], pred_naive)
mae_naive = mean_absolute_error(test_naive['AQI'], pred_naive)

print(f"Random split results:")
print(f"  R-squared = {r2_naive:.3f}")
print(f"  MAE = {mae_naive:.1f}")
print(f"  Train size: {len(train_naive)}, Test size: {len(test_naive)}")
print()
print("Looks great! Ship it?")
```

R² around 0.85 and a low MAE — this looks like a strong model. But there's a hidden problem.

**Think about it**: with a random split, your test set includes days *from the middle* of the time series. A test-set day from June 2022 has its neighbors (June 1, June 3, ...) in the training set. The lag features — "yesterday's AQI" — are computed from data that's *right next to* training points.

This is a form of **data leakage** — not because test data enters the training set directly, but because the random split makes test observations highly correlated with nearby training observations. The lag feature "yesterday's AQI" for a test-set day was likely in the training set.

## Proper backtesting: temporal split

The fix is simple: **train on the past, test on the future**. This mimics how the model would actually be used — you'd train it on historical data and deploy it to predict tomorrow. The diagram below shows the difference: random splitting scatters test points throughout the timeline, while a temporal split draws a clean boundary.
...
**Think about it:** We're about to train on 2021--2022 and test on 2023. Will R-squared be higher, lower, or about the same as the random split? Why?
...
print("=" * 50)
print(f"{'Random split (WRONG)':<25} {r2_naive:>8.3f} {mae_naive:>8.1f}")
print(f"{'Temporal split (RIGHT)':<25} {r2_temporal:>8.3f} {mae_temporal:>8.1f}")
print("=" * 50)
print(f"{'Leakage gap':<25} {r2_naive - r2_temporal:>8.3f} {mae_temporal - mae_naive:>8.1f}")
print()
print("The R-squared gap is the leakage you avoided.")
print("The naive split was lying to you -- the model isn't that good.")
```

The performance drop is substantial. The random split made the model look much better than it actually is. That's the **leakage** — by testing on days surrounded by training data, the model gets to cheat.

Before building a fancy model, we should always check the simplest possible forecast: **tomorrow = today**. This **naive baseline** is the bar any model must clear.
...
This is why backtesting matters. In production, your model only ever sees the past. Your evaluation should reflect that.
...
This is especially important when the data has regime changes — like wildfire season.
...
This is the value of walk-forward validation: it reveals *when* your model breaks, not just how well it does on average.

## Lag features are just feature engineering

Recall from Lecture 6 that feature engineering transforms raw data into more useful predictors. Lag features are the same idea applied to time:
...
## Same lesson, different domain: NBA performance prediction

You might be thinking: "This is an air quality problem. My field is tech/finance/sports." But the temporal leakage trap appears anywhere data has a time dimension. Let's see if the same pattern holds in a completely different domain.

If you could reliably predict how many points an NBA player will score tonight, you could make a fortune betting. Let's try — predicting a player's performance from their last 5 games. Can we randomly split games? No — that would let the model peek at future games.
...
```{python}
# Naive random split vs temporal split
np.random.seed(42)
...
print()
print("Same story -- the random split is optimistically biased.")
```
...
A few terms you'll need: Sportsbooks charge a fee called the **vig** (short for vigorish). At standard **-110 odds**, you risk \$110 to win \$100. That means you need to win about 52.4% of your bets just to break even. A **unit** is one standard bet amount — we'll track profit/loss in units so the math is clean.
...
**Think about it:** Will this strategy make money? Our model has decent R-squared on the temporal split. Is that enough?
...
This is the **backtesting illusion**. In-sample performance does not equal out-of-sample performance. Sportsbooks already factor in rest days, recent form, and home/away. Your model isn't finding an edge — it's rediscovering information the market already knows.

## The surprise: when the future doesn't look like the past
...
**Think about it:** If we train a model on "normal" days (AQI below 300), what would it predict for an AQI of 8,000? How far off would it be?
...
This is **distribution shift** — a severe form of the non-stationarity we saw at the start of this lecture. Not just gradual changes in the mean, but a fundamentally different regime that the training data never captured. It's the deepest problem in time series forecasting:
...
In Lecture 10, we saw that a clinical trial on adults can't tell you about children. Same idea here: a model trained on AQI 0--300 can't tell you what happens at AQI 8,000. You can't extrapolate beyond the range of your data.

In Lecture 17, we'll see that AutoML tools often make this same mistake — they use standard cross-validation by default and don't know your data is temporal.
...
- **Backtesting is not prediction.** A strategy that looks profitable in backtest may fail in production because the market already knows the same signals.
...
- **Data leakage:** When information from the future (or the test set) contaminates training, producing optimistically biased results.
...
- **Walk-forward validation (time series cross-validation):** Repeatedly training on an expanding (or sliding) window of past data and testing on the next period.
- **Distribution shift:** When future data comes from a fundamentally different regime than the training data.

### Key ideas

- Random train/test splits create data leakage in time series because test observations are correlated with nearby training observations.
- Walk-forward validation reveals *when* a model fails, not just its average performance.
- The naive baseline ("same as yesterday") is surprisingly hard to beat and should always be checked.
- Backtesting a strategy does not guarantee future profitability — markets already incorporate public information.
- Distribution shift means models trained on normal conditions cannot predict unprecedented events.
