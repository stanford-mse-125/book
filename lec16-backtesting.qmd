...
A time series is **non-stationary** when its statistical properties (mean, variance) change over time. The AQI data above is non-stationary: the mean and variance shift with the seasons, and extreme events (wildfire spikes) appear unpredictably. We'll explore why this matters throughout this lecture.
:::
...
The **naive forecast** predicts that tomorrow's value equals today's value: $\hat{y}_{t+1} = y_t$. This is the simplest possible forecast and the bar any model must clear. In the forecasting literature, other standard benchmarks include the **seasonal naive** method (predict the value from the same season last year) and the **drift** method (extrapolate the line between the first and last observations). These are collectively called **benchmark forecasting methods**.
:::
...
<!-- FLAG: FPP3 Ch 5 covers residual diagnostics (ACF of residuals, Ljung-Box test) and prediction intervals in depth. This lecture does not address either topic. Consider adding a section on checking residuals (are they white noise?) and on quantifying forecast uncertainty with prediction intervals, or covering them in a separate lecture. -->

<!-- FLAG: FPP3 Ch 5 introduces MAPE and MASE as forecast accuracy measures for comparing across series. This lecture uses only MAE and R-squared. Consider discussing scale-independent error measures if students will encounter multi-series forecasting. -->
...
- **Data leakage:** When information from the future (or the test set) contaminates training, producing optimistically biased results. Random train/test splits create leakage in time series because test observations are correlated with nearby training observations.
