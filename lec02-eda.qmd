...
Suppose you survey children and find that shoe size correlates with reading ability. The correlation is real and reproducible — children with bigger feet genuinely do read better, on average. But bigger shoes don't *cause* better reading. **Age** drives both: older children have bigger feet *and* read better.

The association between shoe size and reading ability is a statistical fact. The mistake would be interpreting it as causal. When you ask whether X *causes* Y, a variable that drives both X and Y creates an association that doesn't reflect a causal effect. That variable is a **confounder**.
...
The ranking reversal illustrates **confounding**. Patient severity is a confounder: it affects both which hospital a patient goes to *and* the readmission outcome. The association between hospital identity and high readmission rates is real — those hospitals genuinely have more readmissions. But interpreting that association as evidence of poor quality is wrong, because patient severity drives both. A hospital can rank among the worst on raw rates yet perform at or below expectations after adjusting for its patient mix.
