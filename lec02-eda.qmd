...
In Chapter 1, we introduced four modes of reasoning with data: summary, prediction, inference, and causation. This chapter focuses on the first: **summary**. Before you predict, infer, or ask causal questions, you need to understand what's in your data. EDA is how you build that understanding.
...
The distributions look roughly symmetric and unimodal, centered near 1.0 (though with a slight right skew — we'll revisit what "normal" really means in Chapter 8). But notice the spread differs by condition — heart failure (HF) has more variation than hip/knee replacement.
...
If you drop these rows, you're analyzing only the large, urban hospitals. Your conclusions would be biased.
:::
...
A heatmap makes the same proportions easier to scan visually:
...
We'll use the Airbnb data to answer a series of increasingly specific questions: What does a typical listing cost? How does price vary across neighborhoods and room types? And are those differences real — or confounded by lurking variables?
...
In our dataset, the `price` column is already stored as integers — we got lucky. But many real-world CSVs aren't so tidy. Here's what raw Airbnb price data sometimes looks like before it's been cleaned:

```{python}
# Simulating raw price data — what Inside Airbnb's CSV sometimes looks like
raw_prices = pd.Series(['$1,200.00', '$85.00', '$250.00', '$0.00', None])
print("Raw format:")
print(raw_prices)
...
The fix: strip the dollar signs and commas, then convert to a numeric type.

```{python}
clean_prices = raw_prices.str.replace(r'[\$,]', '', regex=True).astype(float)
print("After cleaning:")
print(clean_prices)
print(f"\nMean: ${clean_prices.mean():.2f}")
```
...
:::{.callout-tip}
## Think About It
Our dataset has 25 listings at \$0/night. Are those real prices, data entry errors, or something else? How would you decide?
:::
...
For categorical variables, include NaN as its own category:

```{python}
bedrooms_display = airbnb['bedrooms'].copy()
bedrooms_display = bedrooms_display.fillna('Missing')
fig, ax = plt.subplots(figsize=(7, 4))
bedrooms_display.value_counts().sort_index().plot.bar(ax=ax, edgecolor='white')
ax.set_xlabel('Bedrooms')
ax.set_ylabel('Count')
ax.set_title('Bedrooms (including missing)')
plt.tight_layout()
plt.show()
```

For numeric variables, compare distributions for rows where a related column is missing vs. present:

```{python}
airbnb['bedrooms_missing'] = airbnb['bedrooms'].isna()
fig, ax = plt.subplots(figsize=(8, 4))
for label, group in airbnb[airbnb['price'] > 0].groupby('bedrooms_missing'):
    tag = 'Missing bedrooms' if label else 'Has bedrooms'
    sns.histplot(group['price'][group['price'] <= 500], bins=40, ax=ax,
                 label=tag, alpha=0.5, stat='density')
ax.set_xlabel('Price ($)')
ax.set_ylabel('Density')
ax.set_title('Price distribution: missing vs. present bedroom count')
ax.legend()
plt.tight_layout()
plt.show()
```
...
# Panel 2: Log axis — same data, log scale on x-axis
sns.histplot(airbnb_pos['price'], bins=100, ax=axes[1], edgecolor='white', color='seagreen')
axes[1].set_xscale('log')
axes[1].set_xlabel('Price ($)')
axes[1].set_ylabel('Count')
axes[1].set_title('Log axis (preferred for visualization)')
...
The linear histogram (left) is nearly useless — everything is piled up on the left. The log-axis histogram (center) shows the same data on a logarithmic scale: the shape is clear, and the x-axis labels are still in dollars. The log-transformed histogram (right) plots $\log_{10}(\text{price})$ directly — useful when you need to feed the data into a model.
...
With thousands of points, scatter plots become overplotted — every point lands on top of the others and you can't see where the data is concentrated. A **2D histogram** (or heatmap) bins the data in both directions and uses color to show density.
...
# 2D histogram
sns.histplot(data=prices_pos[prices_pos['price'] <= 700],
             x='accommodates', y='price', bins=30, ax=axes[1],
...
| One categorical (NOT) | Pie chart | Almost never — use bar chart instead |
...
Pie charts only make sense when you have a small number of categories (3--5) and the slices represent parts of a meaningful whole. Even then, bar charts are easier to read --- humans judge lengths more accurately than angles. Pie charts make it especially hard to compare categories of similar size. For the Airbnb data, a bar chart of room types is far more readable than a pie chart would be.
:::
...
The dodged chart (left) shows that Manhattan has the most listings overall. The standardized chart (right) reveals something different: Manhattan has a higher *proportion* of entire homes than Brooklyn does. Composition matters.
...
When overlapping histograms get crowded, **faceting** splits the data into separate panels that share the same x-axis. This makes it much easier to compare distributions across groups.

```{python}
prices_filt = airbnb[(airbnb['price'] > 0) & (airbnb['price'] <= 500)]
g = sns.displot(data=prices_filt, x='price', col='room_type',
                col_wrap=2, bins=40, height=3, aspect=1.5)
...
Faceting splits the data into separate panels that share the same x-axis. Comparing price distributions across room types is much easier here than in a single overlapping histogram.
...
Let's go back to the hospital data for the punchline of this chapter.
...
The ranking reversal we just witnessed is a form of **confounding**: patient severity is a lurking variable that distorts the naive comparison. A hospital can rank among the worst on raw rates yet perform at or below expectations after adjusting for its patient mix. We'll study confounding formally in Chapter 18. For now, the lesson is simpler:

:::{.callout-important}
## Definition: Confounding

Two variables are **confounded** when a third variable — the **confounder** — is associated with both, making it impossible to tell from the data alone whether the apparent relationship is real or an artifact. Here, patient severity is a confounder: it affects both which hospital a patient goes to *and* the readmission outcome. Ranking hospitals by raw readmission rates conflates hospital quality with patient severity.
:::
...
We'll revisit this hospital data in Chapter 18 when we ask: *is this causal?* Does the penalty actually reduce readmissions, or does it just punish hospitals that serve vulnerable populations?
...
- **Distributions matter.** The mean can be misleading when distributions are skewed. Always plot a histogram. Use a log axis for heavy-tailed data.
...
- **Confounding makes naive comparisons misleading.** The "worst" hospitals might just be treating the hardest cases. Always ask: what lurking variable could explain this pattern?
...
- **Confounding**: When a third variable is associated with both the predictor and the outcome, distorting the apparent relationship.
