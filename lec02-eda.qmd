...
A bar chart makes the pattern easier to scan across all six conditions at once.
...
We'll use the Airbnb data to answer a series of increasingly specific questions: What does a typical listing cost? How does price vary across neighborhoods and room types? And should we attribute the price variation to the location and room type — or could these differences be confounded by other variables?
...
The fix: replace non-numeric placeholders, strip dollar signs and commas, convert to numeric, and handle invalid values like negatives.
...
The linear histogram (left) is nearly useless — everything is piled up on the left. The log-axis histogram (center) uses logarithmically-spaced bins so that each bar covers an equal range on the log scale — this ensures bar *areas* are proportional to counts, avoiding the visual distortion that equal-width bins create on a log axis. (We use `axes[1].hist()` from matplotlib here instead of `sns.histplot()` because we need custom bin edges.) The log-transformed histogram (right) plots $\log_{10}(\text{price})$ directly — useful when you need to feed the data into a model.
...
Two variables can appear related even when neither causes the other — if a third variable drives both.

Consider a simple example. Suppose you survey children and find that shoe size correlates with reading ability. Bigger feet → better reading? Of course not. **Age** is the lurking variable: older children have bigger feet *and* read better. Shoe size and reading ability are **confounded** by age.

:::{.callout-important}
## Definition: Confounding
Two variables are **confounded** when a third variable — the **confounder** — is associated with both, making the apparent relationship between them misleading. Controlling for the confounder (here, comparing children of the same age) reveals whether the relationship is real.
:::
...
Box plots summarize distributions with five numbers (min, Q1, median, Q3, max). **Violin plots** show the full distribution shape — revealing patterns that box plots hide.
...
The ranking reversal we just witnessed is a form of **confounding** (defined earlier in the Airbnb section): patient severity is a lurking variable that distorts the naive comparison. A hospital can rank among the worst on raw rates yet perform at or below expectations after adjusting for its patient mix. Here, patient severity is the confounder — it affects both which hospital a patient goes to *and* the readmission outcome. We'll study confounding formally in Chapter 18. For now, the lesson is simpler:
