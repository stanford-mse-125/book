...
It's related to the multiple testing problem from Lecture 11: each peek is another chance for a false positive. But it's actually worse, because test statistics at successive peeks are correlated, making standard corrections like Bonferroni overly conservative. Tech companies use sequential testing methods designed for exactly this situation.
...
The standard estimation method for IV is **two-stage least squares (2SLS)**: first regress treatment on the instrument, then regress the outcome on the predicted treatment values. We won't go deep into IV estimation — that's a full course in econometrics — but the idea of finding "clever randomization" in the world is the core of modern causal inference.

### Regression Discontinuity: Exploiting Cutoffs

Another powerful design is **regression discontinuity (RD)** — when treatment is assigned at a sharp cutoff (e.g., a test score threshold for a scholarship), comparing people just above and below the cutoff is like a local randomized experiment. Students scoring 89 vs. 91 on a 90-point threshold are nearly identical, yet one gets the scholarship and the other doesn't. That sharp jump at the cutoff identifies the causal effect. In a **sharp RD**, the cutoff perfectly determines treatment; in a **fuzzy RD**, the cutoff only changes the *probability* of treatment (and the analysis combines RD with IV ideas). A practical choice in any RD is the **bandwidth** — how close to the cutoff observations must be to count. Too wide and you introduce bias; too narrow and you lose precision.
...
:::{.callout-tip}
## Think About It
How would you redesign this NBA analysis to be a *real* DiD? What would be your treatment group, control group, and pre/post periods? What threats to parallel trends would you worry about?
:::
...
- **Instrumental variable (IV)**: A variable that affects treatment assignment but has no direct effect on the outcome except through treatment. Estimated via two-stage least squares (2SLS).
- **Regression discontinuity (RD)**: A design that exploits sharp cutoffs in treatment assignment. Sharp RD has perfect compliance at the cutoff; fuzzy RD has imperfect compliance.
