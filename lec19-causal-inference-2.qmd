...
The idea behind **difference-in-differences (DiD)** is beautifully simple: compare how a treatment group changed relative to a control group. If both groups were on the same trajectory before the treatment, any *divergence* afterward must be caused by the treatment.
...
Because this is simulated data, we know the true effect is -3.0 percentage points and can verify that DiD recovers it. In real data, you never have this luxury — you must rely on the parallel trends assumption.

> **Think about it:** Looking at the plot, how would you quantify the effect of the new protocol? What two comparisons would you make?
...
DiD only works if the treatment and control groups would have followed the **same trajectory** absent the treatment. This is the **parallel trends assumption**.

You can't test this assumption directly (you never observe the counterfactual), but you *can* check whether the groups had parallel trends in the pre-treatment period. Pre-treatment parallel trends are encouraging but don't *prove* the assumption holds — the trends could diverge after treatment for reasons unrelated to the treatment itself. This is the fundamental limitation of DiD.
...
> **Think about it:** What would this event study plot look like if the parallel trends assumption were *violated*? What if Hospital A was already improving faster than Hospital B before the intervention?

### DiD as a Regression

We can estimate DiD with a regression: $Y = \beta_0 + \beta_1 \cdot \text{Treated} + \beta_2 \cdot \text{Post} + \beta_3 \cdot \text{Treated} \times \text{Post} + \varepsilon$. The coefficient $\beta_3$ on the interaction term is the DiD estimate.
...
### John Snow's Natural Experiment: The Most Famous DiD in History

Last lecture we learned that John Snow's most powerful evidence for waterborne cholera was *not* his famous map — it was his comparison of water companies. Let's compute the actual numbers.

In the 1850s, two water companies served intermingled houses on the same streets of South London. Lambeth Company moved its water intake upstream (away from sewage) in 1852. Southwark & Vauxhall kept drawing from the polluted Thames. Because houses were served essentially at random based on which company had laid pipes, this is a **natural experiment** — nature created near-random assignment.
...
print("This is the power of a natural experiment.")
```

Why is this a natural experiment rather than just an observational comparison? Because the two companies served *intermingled* houses on the same streets. A household's water company was determined by which company happened to have laid pipes to that address — essentially arbitrary. This eliminates confounding by neighborhood poverty, crowding, or sanitation.

> **Think about it:** Why can't we frame Snow's water company comparison as a standard DiD with before/after data? (Hint: we only have data from the 1854 epidemic, not from before Lambeth moved its intake.) What assumption are we making instead?

## RCTs: The Gold Standard Revisited

The cleanest way to estimate a causal effect is to **randomly assign** treatment. Recall ACTG 175 from Lecture 10 — the clinical trial that randomized HIV-positive patients into different drug regimens.
...
> **Think about it:** The balance table shows the groups are *nearly* identical, but not *perfectly* identical. Why doesn't randomization guarantee perfect balance? (Hint: think about sample size and the law of large numbers.)

The groups are nearly identical on every baseline characteristic. That's not a coincidence — it's the *design*. Randomization ensures that any confounder, whether we measured it or not, is balanced across groups on average.
...
Your PM checks the dashboard every morning. On day 5, p = 0.03. "Ship it!" they say. But you know better...

**Peeking** — checking results repeatedly and stopping at the first significant result — inflates your false positive rate far beyond 5%. It's related to the multiple testing problem from Lecture 11: each peek is another chance for a false positive. But it's actually worse, because test statistics at successive peeks are correlated, making standard corrections like Bonferroni overly conservative. Tech companies use sequential testing methods designed for exactly this situation.
...
- **Network effects**: If users interact (social networks, marketplaces), treating user A affects user B. The independence assumption fails. This is a violation of **SUTVA** (Stable Unit Treatment Value Assumption) — the assumption that one person's treatment doesn't affect another's outcome.
- **Novelty effects**: Users might click a new design just because it's different. Effects fade over time.
- **Small effects need large samples**: A 0.5% lift in conversion requires millions of users to detect reliably.

> **Think about it:** You're running an A/B test on a social media feed. Treatment users see more local news. But they share those stories with control users. What happens to your estimate?

## Beyond DiD: Other Quasi-Experimental Methods

### Instrumental Variables: A Brief Introduction

Sometimes the confounder is completely unobserved. You can't control for it in regression. One clever trick: find an **instrumental variable (IV)** — something that acts like a coin flip, pushing people toward treatment but having no other path to the outcome.

IV requires two assumptions: (1) **relevance** — the instrument must actually affect the treatment, and (2) **validity** (the exclusion restriction) — the instrument must affect the outcome only through the treatment. In practice, arguing that an instrument is valid is the hardest part. Clever researchers spend most of their effort on this argument.

**The Vietnam draft lottery** (Angrist, 1990):

- The instrument: draft lottery number (randomly assigned)
- Affects treatment: low numbers were drafted into military service
- No direct effect: your lottery number doesn't affect your earnings except through whether you served

We won't go deep into IV estimation — that's a full course in econometrics — but the idea of finding "clever randomization" in the world is the core of modern causal inference.

### Regression Discontinuity: Exploiting Cutoffs

Another powerful design is **regression discontinuity (RD)** — when treatment is assigned at a sharp cutoff (e.g., a test score threshold for a scholarship), comparing people just above and below the cutoff is like a local randomized experiment. Students scoring 89 vs. 91 on a 90-point threshold are nearly identical, yet one gets the scholarship and the other doesn't. That sharp jump at the cutoff identifies the causal effect.

We won't work through RD in detail, but you should know it exists as a major tool in the quasi-experimental toolkit alongside DiD and IV.

## The Surprise: An NBA Natural Experiment
...
Here's the honest takeaway: this analysis is *descriptive*, not causal. We compared star minutes early vs. late in the season and across seasons, but we didn't cleanly separate teams affected by the play-in from those unaffected. The "treatment" isn't sharply assigned — a team's proximity to the cutoff changes game by game. And many things change between seasons besides the play-in tournament.

This is common in practice. Causal inference is as much about acknowledging what your data *can't* tell you as about producing estimates. Contrast this with Snow's water companies, where the natural experiment was clean, or the ACTG 175 trial, where randomization made the causal inference airtight.

> **Think about it:** How would you redesign this NBA analysis to be a *real* DiD? What would be your treatment group, control group, and pre/post periods? What threats to parallel trends would you worry about?
...
**Further reading:** For more on DiD and natural experiments, see Huntington-Klein, *The Effect*, Chapters 16-18.
...
- **Regression discontinuity (RD)**: A design that exploits sharp cutoffs in treatment assignment.

### Key Ideas

- DiD removes confounding from shared time trends by differencing twice: within groups across time, then across groups.
- Randomization is powerful because it closes all backdoor paths in the DAG — no confounder adjustment needed.
