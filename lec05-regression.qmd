...
The first row is the model we just fit. The second row explains what happens when you regress $\log(\text{price})$ on bedrooms: the coefficient lives on the log scale, so exponentiating converts it to a multiplicative factor. Let's see this concretely.
...
model_log.fit(df_model[['bedrooms']], df_model['log_price'])
